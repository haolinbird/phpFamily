...
class Handler {
    ...
    protected $loggerCfg = null;

    protected $initLogDir = "/var/log/";
    //　压测路径　写死了．　不会被采集到
    protected $initBenchDir = "/home/logs/x-jumei-bench/";
    ...
    protected function __construct($cfgName) {
        if(!static::$configs)
        {
            static::$configs = (array) new \Config\Log;
        }
        if (!isset(static::$configs[$cfgName]))
            return false;
        $cfg = static::$configs[$cfgName];
    ...
    protected function fileLogger($msg) {
        if (isset($this->loggerCfg['path'])) {
            $logDir = dirname($this->loggerCfg['path']) . DIRECTORY_SEPARATOR;
            $fileName = basename($this->loggerCfg['path']);
        } else {
            /*$logDir = static::$configs['FILE_LOG_ROOT'] . DIRECTORY_SEPARATOR . $this->cfgName . DIRECTORY_SEPARATOR;*/
            $baseDir = isset(static::$configs['FILE_LOG_ROOT']) ? static::$configs['FILE_LOG_ROOT'] : $this->initLogDir;
            $logDir = $baseDir . DIRECTORY_SEPARATOR . $this->cfgName . DIRECTORY_SEPARATOR;
            $fileName = $this->cfgName . '.log';
        }

        // 压测日志强制写入此路径：之前的配置取消
        if (\JmArchiTracker\Tracker::isBench()) {
            $logDir = $this->initBenchDir . DIRECTORY_SEPARATOR . $this->cfgName . DIRECTORY_SEPARATOR;
        }

        if (! is_dir ( $logDir ) && ! @mkdir ( $logDir, 0777, true ) && !is_writable ( $logDir )) {
            return false;
        }
        if(isset($this->loggerCfg['rotateFormat'])){
            $extPos = strrpos($fileName, '.');
            if($extPos === 0){// prettify filenames like "/.log"
                $offset = $extPos;
            } else {
                $offset = $extPos + 1;
            }

            $fileName = substr($fileName, 0, $offset).date($this->loggerCfg['rotateFormat']).substr($fileName, $extPos);
        }
        if($this->useRawMessage) {
            $msg = $this->formatMessage($msg);
        }

        file_put_contents ( $logDir . $fileName, $msg, FILE_APPEND );
    }
    ...
    {
        if (!isset(self::$config[$cfgName]))
            return false;
        if (! in_array ( self::$config[$cfgName]['logger'], self::$availableLoggers )) {
            return false;
        }
        return true;
    }
