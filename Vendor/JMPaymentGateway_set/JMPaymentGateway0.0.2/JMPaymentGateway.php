...
class JMPaymentGateway
{
    ...
    public function buildRequestPara($params)
    {
        ...
        if (isset($params['user']) || isset($params['timestamp'])) {
            throw new \Exception('ILLEGAL_REQUEST_ARGS');
        }
    ...
    protected function remoteRequest(array $data)
    {
        ...
        if ($this->requestMethod == 'GET') {
            $response = $this->getHttpResponseGET($this->gatewayUrl, $data, $this->timeOut, $error);
        } else {
            $response = $this->getHttpResponsePOST($this->gatewayUrl, $data, $this->timeOut, $error);
        }
        if ($error) {
        ...
        if ($response === false || !$ctx) {
            throw new \Exception(
                sprintf(
                    'JMPaymentGateway: Network %s may time out (%.3fs), or there ara fatal errors on the server',
                    $this->gatewayUrl,
                    $this->executionTime()
                )
            );
