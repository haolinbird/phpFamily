...
abstract class RedisBase {
     ...
     * @var static
     */
    protected $partitionInstance=null;
    ...
    /**
     * 是否已经初始化. (主要用于标识配置是否已经处理)
     * @var bool
     */
    protected $intialized;
    ...
    protected function __construct() {
        ...
        $this->haConfigManager = new \SharedMemory\HaConfig($IPCKey);
    }
    ...
    public function partitionInfo(){
        $info = array();
        if(!isset(static::$configs['partitionConfig']['partition'])){
            return $info;
        }

        foreach(static::$configs['partitionConfig']['partition'] as $n => $p){
     ...
     * 调用partitionByUID($uid)后, 执行接下来的redis操作时，将使用UID对应分区的连接.
     * （当UID等于0时，即使用默认分区)。
     * 用法: \Redis\RedisMultiCache::getInstance('default')->partitionByUID(211)->hGetAll('user_info:211')
     * 或 \Redis\RedisCache::getInstance('default')->partitionByUID(211)->hGetAll('user_info:211')
     * </pre>
     * @param int $uid
     * @return $this
     * @throws Exception
    ...
    public function partitionByUID($uid){
        if(!isset(static::$configs['partitionConfig']) || !is_array(static::$configs['partitionConfig'])){
            throw new Exception('分区配置(partitionConfig)不存在,请检查配置!');
        }
        $partitionConfig = static::$configs['partitionConfig'];
        if(!isset($partitionConfig['partition']) && !is_array($partitionConfig['partition'])) {
            throw new Exception('分区配置为空，请检查配置文件！');
        }
        if(!isset(static::$configs[$this->configName]['cluster_name'])){
            throw new Exception('配置中未指定集群名称(cluster_name). 当使用UID分区时，请在对应的配置中指定集群名称！');
        }
        $clusterName = static::$configs[$this->configName]['cluster_name'];
        $config = null;
        $partitionFound = false;
        foreach ($partitionConfig['partition'] as $n => $p){
            if($partitionFound){
                break;
            }
            if(!isset($p['uid_range'])){
                throw new Exception("分区 {$n} 未指定UID区间(uid_range)！");
            }
            if(!is_array($p['uid_range'])){
                throw new Exception("分区 {$n} 的UID区间(uid_range) 必须为二维数组！");
            }
            foreach ($p['uid_range'] as $range){
                if(!is_array($range)){
                    throw new Exception("分区 {$n} 的UID区间(uid_range) 必须为二维数组！");
                }

                if(count($range ) != 2){
                    throw new Exception("分区 {$n} 的UID区间(uid_range) 必须设置上下限！");
                }

                if($range[1] < $range[0]){
                    throw new Exception("分区 {$n} 的UID区间(uid_range) 上限必须大于下限！");
                }
                ...
                unset($p['uid_range']);
                foreach ($p as $cluster => $clusterConfig){
                    if($cluster == $clusterName){
                        $config = $clusterConfig;
                    }
                }
                if(!$config){
                    throw new Exception("UID定位到分区 {$n}，但未找到集群{$clusterName}的配置！");
                }
                $partitionFound = true;
                break;
            }
        ...
        if(!$config){
            throw new Exception("UID({$uid})未找到匹配的分区！");
        }
        if($this->partitionInstance === null){
            $this->partitionInstance = new static();
            $ins = $this->partitionInstance;
            $ins::$configs = $this::$configs;
            $this->partitionInstance->config = $this->config;
            $this->partitionInstance->configName = $this->configName;
        }

        $this->partitionInstance->partitionIndex = $n;
        $this->partitionInstance->config['nodes'] = $config;
        $this->partitionInstance->intialized = false;
        return $this->partitionInstance;
    }

    protected $target = '';
    protected  function realCallRedisCmd($name, $arguments){
    ...
    /**
     * 执行任何redis命令之前调用此方法，以初始化或处理分区配置.
     */
    ...
    public function __call($name, $arguments) {
        $this->tryInitialize();
        $anotherRetry = 1;
        for(;$anotherRetry >= 0; $anotherRetry--){
            try{
               return $this->realCallRedisCmd($name, $arguments);
            }catch(\Exception $ex){
                if(strpos($ex->getMessage(), 'bad connection') !== false || strpos($ex->getMessage(), 'Connection lost') !== false){
                    continue;
                }
                throw $ex;
            }
    ...
    public function real_connect($target, $key) {
        ...
        if (false == $this->_ConnectTarget($target, $ip, $port, $timeout, $err)) {
            $logger->REDIS_CR('exception', 0, "redis连接失败:[$err]，服务器地址:$target");
            unset($this->redis[$target]);
            throw new Exception("redis链接错误:[$err]。\n目标地址:$target\nkey:$key\ndb号:{$this->config['db']}");
        }
...
//        if ((int) $redis->socket === (int) $redisTarget->socket) {//key1,key2刚好在一台机器
     ...
     * @return bool
     */
    protected function _ConnectTarget($target, $ip, $port, $timeout, &$err) {
        // 如果状态为链接池
        ...
        try {
            // 直连, 状态只能为CONNECT_FAILED, CONNECT_DIRECT。
