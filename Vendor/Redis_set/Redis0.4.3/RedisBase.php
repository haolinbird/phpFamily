...
abstract class RedisBase {
    ...
    */
    ...
    public function __call($name, $arguments) {
        if ($this->isTransaction || $this->isPipeline) {//事務緩存
    ...
    {
        // 只有随机选择模式才能使用pipeline.
        if (! isset($this->config['consistent']) || $this->config['consistent'] == true) {
            throw new Exception("Pipe error! This feature cannot be used by the current configuration mode");
        }
    ...
    {
        // 只有随机选择模式才能使用transaction.
        if (! isset($this->config['consistent']) || $this->config['consistent'] == true) {
            throw new Exception("Transaction error! This feature cannot be used by the current configuration mode");
        }
        ...
        if (!isset($this->redis[$target])) {//每个物理机对应一个new redis
            try {
                ...
                if (strstr($e->getMessage(), "connect to pool_server fail")) {//端口连不上
                    ...
                    throw new Exception('Connect redis error:'.$e->getMessage()."!\nKey: " . $key . "\nTarget:" . $target . "\nDB:" . $this->config['db']);
                }
            }

            if (isset($this->config['password'])) {
                $this->redis[$target]->auth($this->config['password']);
            }
        }
    }
