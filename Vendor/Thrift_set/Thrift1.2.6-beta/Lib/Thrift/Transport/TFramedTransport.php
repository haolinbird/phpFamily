...
//  private $transport_;
  ...
  public function setSpecifiedAddress($specified_address) {
...
//    $this->transport_->write(pack('N', TStringFuncFactory::create()->strlen($this->wBuf_)).$this->wBuf_);
      ...
      if ($this->serverType == self::CONN_TYPE_LARK ) { // 客户端连接的lark
          $timeout = \Thrift\Context::get('thrift_recv_timeout') == null ? 30 : \Thrift\Context::get('thrift_recv_timeout');
          $context = array(
                      'recv_timeout'    => $timeout,
                      'client_service'  => 'thrift_client',
                      'service_name'    => $this->service_,
                    );
          if (!empty($this->specified_address)) {
              $context['target_address'] = $this->specified_address;
          }
          $buffer = Lark::encode($this->tag_, array($buffer, $this->service_, json_encode($context)));
      } elseif ($this->tag_ == Lark::TAG_RPC_SERVER_SEND){ // $this->tag_ == Lark::TAG_RPC_SERVER_SEND 说明是在服务端，必须要按照TLV协议返回
  ...
  public function writeContextSerialize(){
    ...
    $obj = new \Thrift\ContextSerialize($context);
