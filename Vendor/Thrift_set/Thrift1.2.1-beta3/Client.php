...
class Client 
{
    ...
    public static function instance($serviceName, $newOne = false, $rawThrift = false)
    {
        ...
        if(isset(self::$config[$project]['protocol']) && self::$config[$project]['protocol'] == 'text')
        {
            ...
            $config = array(
                ...
                $service => array(
                    ...
                    'service' => isset(self::$config[$project]['service']) ? self::$config[$project]['service'] : $project,
                    'dove_key' => isset(self::$config[$project]['dove_key']) ? self::$config[$project]['dove_key'] : null,
                )
            );
        ...
        \Thrift\Context::put('use_connect_pool', false);
        \Thrift\Context::put('rpc_context', json_encode(array(
            'service_provider_name' => isset(self::$config[$project]['service']) ? self::$config[$project]['service'] : $project,
            'dovekey' => isset(self::$config[$project]['dove_key']) ? self::$config[$project]['dove_key'] : null,
        )));
        ...
        {
            self::$instance[$serviceName] = new ThriftInstance($service, $project, $rawThrift);
        }
    ...
    public static function getOneAddress($service_name)
    {
        ...
        // 总的节点列表
        $address_list_with_weight = self::$config[$service_name]['nodes'];
        $address_list = $address_list_weight_map = array();
        $base_weight = 20;
        $total_weight = 0;
        foreach($address_list_with_weight as $node){
            $node_portions = explode(':', $node);
            $ip_port = $node_portions[0].':'.$node_portions[1];
            $address_list[] = $ip_port;
            $total_weight += $address_list_weight_map[$ip_port] = isset($node_portions[2]) ? (int)$node_portions[2] : 1;
        }
        // owl trace 
        global $owl_context;
        ...
        $sample_key_list = array();
        $sample_length = 0;
        foreach($address_list as $k => $v){
            $weight = 1;
            if(isset($address_list_weight_map[$v])){
                $weight = $address_list_weight_map[$v];
            }
            $c_weight = ceil($base_weight * $weight / $total_weight);
            $sample_key_list = array_pad($sample_key_list, $sample_length += $c_weight, $k);
        }
        // 随机选择一个节点
        return $address_list[$sample_key_list[array_rand($sample_key_list)]];
    }
    ...
    public static function getShmFd()
    {
        ...
        {
            self::$badAddressShmFd = shm_attach(crc32(__FILE__));
        }
     ...
     * 发送报警信息.
     *
     * @param mixed  $alarm_data 该参数已经废弃, 为了兼容业务方调用, 从而保留.
    ...
    public static function sendSmsAlarm($alarm_data, $content)
    {
        ...
        foreach($phone_array as $phone)
        {
            if ($phone) {
                ...
                // self::$logger->logInfo('send msg ' . $phone . ' ' . $content. ' send_ret:' .var_export($response, true));
            }
        }
