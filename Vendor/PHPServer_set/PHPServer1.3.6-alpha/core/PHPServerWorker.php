...
class PHPServerWorker
{
     ...
     * @var string
     */
    public static $traceStr = null;

    /**
     * 是否为业务的起始节点，主要是用来跟踪使用.
     *
     * @var boolean
     */
    public static $isActionStartNode = false;
     ...
     */
    public function __construct($socket = null, $recv_timeout = 1000, $process_timeout = 30000, $send_timeout = 1000, $persistent_connection = false, $max_requests = 0, $project_name = '', $max_work_time = null)
    {
        // 初始化
        ...
        switch($signal)
        {
            ...
            case SIGALRM:
                ...
                if(self::$currentModule)
                {
	                ...
	                $e = new Exception("process_timeout({$this->processTimeout}ms)", 504);
	                $this->notice($e->getMessage().":\n".$e->getTraceAsString());
	                throw $e;
                }
                break;
     ...
     * @return boolean|null
     */
    protected function startTingYunAction($class, $method, $traceStr = null)
    {
     ...
     * @return boolean|null
     */
    protected function endTingYunAction()
    {
        if (!$this->isTingYunAgentOn()) {
            return false;
        }
     ...
     * @return void
     */
    protected function noticeTingYunError(\Exception $e)
    {
        if (!$this->isTingYunAgentOn()) {
            return;
        }
     ...
     * @return string
     */
    protected function startTingYunWebService($url)
    {
        if (!$this->isTingYunAgentOn()) {
            return '';
        }
     ...
     * @return string
     */
    protected function endTingYunWebService()
    {
        if (!$this->isTingYunAgentOn()) {
            return '';
        }
        if (function_exists('\tingyun_end_webservice')) {
    ...
    public function isTingYunAgentOn() {
        return PHPServerConfig::get('tingyun_agent_on', 'rpc');
    }
}
