...
class PHPServerWorker
{
    ...
    const CONN_PERSISTENT  = 'persistent-connection';

    const CONN_MULTI       = 'multi-connection';

    /**
     * fd对应的链接类型（长|短）
     */
    protected $fdConnType = array();
    ...
    public function stopServe($exit = true) {
        //直接关闭来自lark的链接（lark会hold链接，server主动断开后，
        //如果lark复用该connection会造成一次错误，但会再次重试，保证业务不受影响）
        foreach ($this->connections as $fd=>$connctions) {
             if (@$this->fdConnType[$fd] === self::CONN_PERSISTENT) {
                $this->closeClient($fd);
            }
        }
    ...
    public function accept($socket) {
        ...
        if (self::isRpcWorker(get_called_class())) {
            // 保证进程只接一个连接
            $this->event->del($this->mainSocket, BaseEvent::EV_ACCEPT);
            // rpc协议本次cgi都采用长链接方式通信
            $this->fdConnType[$fd] = self::CONN_PERSISTENT;
        }
    ...
    public function dealInputBase($connection, $length, $buffer, $fd = null)
    {
        ...
        if(0 === $remain_len) {
            ...
            do {
                ...
                // 长连接只需要清空掉buffer
                if ($this->isPersistentConnection || @$this->fdConnType[$fd] === self::CONN_PERSISTENT) {
    ...
    protected function closeClient($fd) {
        ...
        if($this->protocol != 'udp')
        {
            ...
            unset($this->connections[$fd], $this->recvBuffers[$fd], $this->fdConnType[$fd]);
            $this->statusInfo['connections'] = count($this->connections);
        }
    }
    ...
    public static function isRpcWorker($workerClass) {
