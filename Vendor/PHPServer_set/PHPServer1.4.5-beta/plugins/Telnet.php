    ...
    public static function dealCmd($connection, $length, $buffer, $fd = null)
    {
        ...
        switch($buffer)
        {
            ...
            case 'status':
                ...
                self::sendToClient('Version:'.PHPServer::VERSION."\n");
                ...
                self::sendToClient("pid\tmemory    proto  port  timestamp  ".str_pad('worker_name', self::$maxWorkerNameLength)." " . str_pad('connections', 11) . ' ' .str_pad('total_request', 13)." ".str_pad('recv_timeout', 12)." ".str_pad('proc_timeout',12)." ".str_pad('packet_err', 10)." ".str_pad('thunder_herd', 12)." ".str_pad('client_close', 12)." ".str_pad('send_fail', 9)." ".str_pad('throw_exception', 15)." suc/total\n");
                break;
                // 停止server
            case 'stop':
                PHPServer::stop();
            ...
            case 'reload':
                ...
                $pids2Restart = array_keys(PHPServer::getPidWorkerNameMap(array('ServiceRegister')));
                PHPServer::addToRestartWorkers($pids2Restart);
                PHPServer::restartWorkers();
