...
class StatisticProvider extends PHPServerWorker
{
     ...
     * @var unknown_type
     */
    const SUB_CMD_GET_CLIENT_SUMMARY = 205;
    
    /**
     * 获得单个ip接口访问曲线数据
     * @var unknown_type
     */
    const SUB_CMD_GET_CLIENT_DETAIL = 206;
    
    /**
     * 获得客户端原始统计数据
     * @var unknown_type
     */
    const SUB_CMD_GET_CLIENT_RAW = 207;
    
     /**
     * 获得各个用户的接口访问量
     * @var unknown_type
     */
    const SUB_CMD_GET_CLIENT_USER_SUMMARY = 208;
    
    /**
     * 获得单个用户接口访问曲线数据
     * @var unknown_type
     */
    const SUB_CMD_GET_CLIENT_USER_DETAIL = 209;
    
    /**
     * 获得用户客户端原始统计数据
     * @var unknown_type
     */
    const SUB_CMD_GET_CLIENT_USER_RAW = 210;
            ...
            case self::CMD_PROVIDER:
                switch ($sub_cmd)
                {
                    ...
                    case self::SUB_CMD_GET_LOGS:
                        ...
                        $this->sendToClient($pack_data->getBuffer());
                        return;
                    case self::SUB_CMD_GET_CLIENT_SUMMARY:
                    case self::SUB_CMD_GET_CLIENT_USER_SUMMARY:
                    	$module_interface = json_decode($pack_data->body, true);
                    	$module = isset($module_interface['module']) ? $module_interface['module'] : '';
                    	$interface = isset($module_interface['interface']) ? $module_interface['interface'] : '';
                    	$time = isset($module_interface['time']) ? $module_interface['time'] : '';
                    	$pack_data->body = json_encode(array(
                    			'statistic' => $this->getClientSummary($module, $interface, $time, $sub_cmd),
                    	));
                    	$this->sendToClient($pack_data->getBuffer());
                    	return;
                    case self::SUB_CMD_GET_CLIENT_DETAIL:
                    case self::SUB_CMD_GET_CLIENT_USER_DETAIL:
                    	$module_interface = json_decode($pack_data->body, true);
                    	$module = isset($module_interface['module']) ? $module_interface['module'] : '';
                    	$interface = isset($module_interface['interface']) ? $module_interface['interface'] : '';
                    	$time = isset($module_interface['time']) ? $module_interface['time'] : '';
                    	$ip = isset($module_interface['ip']) ? $module_interface['ip'] : '';
                    	$pack_data->body = json_encode(array(
                    			'statistic' => $this->getClientDetail($module, $interface, $ip, $time, $sub_cmd),
                    	));
                    	$this->sendToClient($pack_data->getBuffer());
                    	return;
                    case self::SUB_CMD_GET_CLIENT_RAW:
                    case self::SUB_CMD_GET_CLIENT_USER_RAW:
                    	$module_interface = json_decode($pack_data->body, true);
                    	$module = isset($module_interface['module']) ? $module_interface['module'] : '';
                    	$interface = isset($module_interface['interface']) ? $module_interface['interface'] : '';
                    	$time = isset($module_interface['time']) ? $module_interface['time'] : '';
                    	$pack_data->body = json_encode(array(
                    			'statistic' => $this->getClientRaw($module, $interface, $time, $sub_cmd),
                    	));
                    	$this->sendToClient($pack_data->getBuffer());
                    	return;
                }
                $this->sendToClient('bad package');
                return;
        }
    }
    ...
    public function getModules($current_module = '')
    {
        ...
        foreach($this->glob($st_dir."/*") as $module_file)
        {
            ...
            if($current_module == $module)
            {
                ...
                $all_interface = array();
                foreach($this->glob($st_dir."*") as $file)
                {
                    if(is_dir($file))
                    {
                        continue;
     ...
     * @return bool/string
     */
    protected function getClientSummary($module, $interface, $start_time='', $cmd)
    {
    	if(empty($module) || empty($interface))
    	{
    		return '';
    	}
    	if($cmd == self::SUB_CMD_GET_CLIENT_USER_SUMMARY)
    	{
    		$detail_file = SERVER_BASE."logs/statistic/userdetail/{$module}/{$interface}-detail|". ($start_time === '' ? date('Y-m-d') : date('Y-m-d', $start_time));
    	}
    	else 
    	{
    	    // log文件
    	    $detail_file = SERVER_BASE."logs/statistic/detail/{$module}/{$interface}-detail|". ($start_time === '' ? date('Y-m-d') : date('Y-m-d', $start_time));
    	}
    	// [ip:[suc_cnt,fail_cnt], ip:[suc_cnt,fail_cnt], ..]
    	$ip_data = array();
    	$ret =  @file_get_contents($detail_file);
    	if($ret)
    	{
    		$tmp = explode("\n", $ret);
    		foreach($tmp as $line)
    		{
    			...
    			$tmp2 = explode("\t", $line);
    			$time = $tmp2[0];
    			$detail = json_decode($tmp2[1], true);
    			if($detail)
    			{
    				foreach($detail as $ip=>$count_data)
    				{
    					if(!isset($ip_data[$ip]))
    					{
    						$ip_data[$ip] = array(0,0);
    					}
    					$ip_data[$ip][0] += $count_data[0];
    					$ip_data[$ip][1] += $count_data[1];
    				}
    			}
    		}
    	}
    ...
    protected function getClientDetail($module, $interface, $ip, $start_time='', $sub_cmd=self::SUB_CMD_GET_CLIENT_DETAIL)
    {
    	if(empty($module) || empty($interface) || empty($ip))
    	{
    		return '';
    	}
    	if($sub_cmd == self::SUB_CMD_GET_CLIENT_USER_DETAIL)
    	{
    		$detail_file = SERVER_BASE."logs/statistic/userdetail/{$module}/{$interface}-detail|". ($start_time === '' ? date('Y-m-d') : date('Y-m-d', $start_time));
    	}
    	else 
    	...
    	if($ret)
    	{
    		...
    		foreach($tmp as $line)
    		{
    		    ...
    		    $count_data = $detail[$ip];
    		    if(!isset($ip_data[$time]))
    		    {
    				$ip_data[$time] = array(0,0);
    			}
    			$ip_data[$time][0] += $count_data[0];
    			$ip_data[$time][1] += $count_data[1];
    		}
    	}
    	return $ip_data;
    }
    
    protected function getClientRaw($module, $interface, $start_time='', $sub_cmd)
    {
    	if(empty($module) || empty($interface))
    	{
    		return '';
    	}
    	if($sub_cmd == self::SUB_CMD_GET_CLIENT_USER_RAW)
    	{
    		$detail_file = SERVER_BASE."logs/statistic/userdetail/{$module}/{$interface}-detail|". ($start_time === '' ? date('Y-m-d') : date('Y-m-d', $start_time));
    	}
    	else 
    	...
    	return @file_get_contents($detail_file);
    }
}
