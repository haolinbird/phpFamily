...
abstract class RpcWorker extends PHPServerWorker implements IWorker
{
    ...
    protected function checkQuota($class, $method, $user)
    {
    	...
    	$this->connectToQuotaAgent();
    	if($this->quotaAgentConnection)
    	{
    	    fwrite($this->quotaAgentConnection, json_encode(array('class'=>$class, 'method'=>$method, 'user'=>$user))."\n");
            $ret = fread($this->quotaAgentConnection, 1024);
	    	if($ret === '0' && !empty($this->quotaConfig[$this->projectName][$class][$method][$user]['strict']))
	    	{
    ...
    protected function connectToQuotaAgent()
    {
        ...
        if(!is_resource($this->quotaAgentConnection) || feof($this->quotaAgentConnection) || feof($this->quotaAgentConnection))
    	{
    	    $this->quotaAgentConnection = stream_socket_client('tcp://127.0.0.1:'.$port, $errno, $errmsg, 1/*, STREAM_CLIENT_CONNECT | STREAM_CLIENT_PERSISTENT*/);
    	    @stream_set_timeout($this->quotaAgentConnection, 0, 1000);
    	}
    }
