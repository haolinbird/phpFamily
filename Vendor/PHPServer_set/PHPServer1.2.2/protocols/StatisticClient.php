...
class StatisticClient
{
    ...
    // 上报地址
    protected static $reportAddress = 'udp://127.0.0.1:20205';
     ...
     * @return true/false
     */
    public static function report($module, $interface, $code = 0, $msg = '', $success = true, $source_ip = '', $target_ip = '', $user = 'not-set')
    {
        ...
        //花费的时间(发现microtime(true)有bug，偶尔不准确)
        $cost_time = $time_now - $time_start;
        if($cost_time > 500 || $cost_time < 0)
        {
        	return;
        }
        ...
        return self::sendData($data.$module.$interface.$msg.$user, self::$reportAddress);
    }
     ...
     * @return bool
     */
    private static function sendData($bin_data, $address)
    {
