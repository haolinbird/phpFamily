...
class PHPServer
{
    ...
    public static function checkExtension()
    {
        ...
        echo "----------------------EXTENSION--------------------\n";
        $pad_length = 32;
        echo str_pad('php', $pad_length), "\033[32;40m [".PHP_VERSION."] \033[0m\n";
    ...
    public static function checkWorkersConfig()
    {
        ...
        foreach (PHPServerConfig::get('workers') as $worker_name=>$config)
        {
            $worker_name_display = $worker_name.'@'.$config['port'];
            // 端口、协议、进程数等信息
            if(empty($config['child_count']))
            {
                ServerLog::add(str_pad($worker_name_display, $pad_length)." [child_count not set]\tServer start fail");
                exit(str_pad($worker_name_display, $pad_length)."\033[31;40m [child_count not set]\033[0m\n\n\033[31;40mServer start fail\033[0m\n");
            }
            
            $total_worker_count += $config['child_count'];
            
            // 计算最长的worker_name
            if(self::$maxWorkerNameLength < strlen($worker_name_display))
            {
                self::$maxWorkerNameLength = strlen($worker_name_display);
            }
            ...
            if(0 != self::checkSyntaxError($worker_name))
            {
                ...
                self::notice("$worker_name_display has Fatal Err");
                echo str_pad($worker_name_display, $pad_length),"\033[31;40m [Fatal Err] \033[0m\n";
                break;
            }
            echo str_pad($worker_name_display, $pad_length),"\033[32;40m [OK] \033[0m\n";
        }
