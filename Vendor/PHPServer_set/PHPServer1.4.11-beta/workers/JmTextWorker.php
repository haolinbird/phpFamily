...
class JmTextWorker extends RpcWorker
{
    ...
    public function onServe()
    {
        ...
        try{
            ...
            ServerLog::add("worker [{$this->workerName}] get TraceLogger instance failed: " . $e->getMessage());
        }

        try{
            self::$exLogger = MNLogger\EXLogger::instance();
        } catch(Exception $e){
            ServerLog::add("worker [{$this->workerName}] get ExLogger instance failed: " . $e->getMessage());
        }
        ...
        } else {//没有配置则使用本地StatisticWorker中的配置
