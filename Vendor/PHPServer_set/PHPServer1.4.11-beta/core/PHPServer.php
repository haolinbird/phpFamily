...
include SERVER_BASE . 'plugins/Utility.php';
...
class PHPServer {
    ...
    public static function run($worker_user_name = '')
    {
        ...
        if(!(getenv('PHPSERVER_NODAEMON') == 'true')){
            // 使之成为daemon进程
            self::daemonize();
        }
    ...
    protected static function createSocketsAndListen() {
        // backlog
        $backlog = (int)PHPServerConfig::get('backlog');
        if($backlog <= 0) {
            $backlog = 10240;
        }
        ...
        foreach (PHPServerConfig::get('workers') as $worker_name=>$config) {
            ...
            $address = "{$config['protocol']}://{$ip}:{$config['port']}";

            if ((!isset($config['disable_lark']) || $config['disable_lark'] === false) && !empty($config['worker_class']) && PHPServerWorker::isRpcWorker($config['worker_class'])) {
                $address = Utility::getCgiPass($worker_name);
                $sockFile = substr($address, 7);
                if (file_exists($sockFile)) {
                    unlink($sockFile);
                }
                if (strpos($address, 'unix://') === 0 && is_file(substr($address, 7))) {
                    exit("$address is already create\n");
                }
            }

            // 创建监听socket
            self::$listenedSockets[$worker_name] = stream_socket_server($address, $error_no, $error_msg, $flags, $context);

            if(!self::$listenedSockets[$worker_name]) {
                ServerLog::add("can not create socket $address info:{$error_no} {$error_msg}\tServer start fail");
                exit("\n\033[31;40mcan not create socket $address info:{$error_no} {$error_msg}\033[0m\n\n\033[31;40mServer start fail\033[0m\n\n");
            }
        }
        $port = getenv('PHPSERVER_CONTROL_CMD_PORT');
        if(!$port){
            $port = '10101';
        }
        // 创建telnet监听socket
        Telnet::bindAndListen('0.0.0.0', $port, self::$event);
    }
    
    protected static function initProjectName() {
        self::$projectName = PHPServerConfig::get('project_name');
    }
    ...
    protected static function forkOneWorker($worker_name)
    {
        ...
        elseif($pid === 0)
        {
            ...
            $slowlog_time = PHPServerConfig::get('workers.' . $worker_name . '.slowlog_time');
            $slowlog_time = (int)$slowlog_time > 0 ? (int)$slowlog_time : self::WORKER_DEFAULT_SLOWLOG_TIME;
            ...
            // 是否开启长连接
            $persistent_connection = PHPServerConfig::get('workers.' . $worker_name . '.persistent_connection');
            $max_requests = (int)PHPServerConfig::get('workers.' . $worker_name . '.max_requests');
            $max_work_time = (int)PHPServerConfig::get('workers.' . $worker_name . '.max_work_time');
            ...
            $worker = new $class_name(
                isset(self::$listenedSockets[$worker_name]) ? self::$listenedSockets[$worker_name] : null,
                $recv_timeout,
                $process_timeout,
                $send_timeout,
    ...
    protected static function installSignal()
    {
        ...
        self::$event->add(SIGTERM, BaseEvent::EV_SIGNAL, array('PHPServer', 'signalHandler'), SIGTERM);
        // 设置SIGUSR1信号处理函数,测试用
    ...
    public static function signalHandler($null, $flag, $signal)
    {
        switch($signal)
        {
            ...
            case SIGUSR2:
                ...
                // reload不应该包括服务注册进程，否则可能在发代码时会导致一瞬间所有服务都下线
                $pids2Restart = array_keys(self::getPidWorkerNameMap(array('ServiceRegister')));
                self::addToRestartWorkers($pids2Restart);
                self::restartWorkers();
                break;
        }
    }
     ...
     *
     * @param $filterWorkerName array 不需要组装的服务名字
     *
     * @return array('pid1'=>'worker_name1','pid2'=>'worker_name2', ...)
     */
    public static function getPidWorkerNameMap($filterWorkerName = array())
    {
        $all_pid = array();
        foreach(self::$workerPids as $worker_name=>$pid_array) {
            if (in_array($worker_name, $filterWorkerName)) {
                continue;
            }
    ...
    protected static function checkEnv()
    {
        ...
        // 检查运行时状态目录
        if (!self::checkRuntimeDir()) {
            ServerLog::add("runtime dir [" . self::RUNTIME_DIR ."] can't not be read/write by user " . self::$workerUserName);
            exit("runtime dir [" . self::RUNTIME_DIR ."] can't not be read/write by user " . self::$workerUserName . "[0m\n\n\e[31;40mServer start fail\e[0m\n\n");
        }
    }
     ...
     * @return boolean
     */
    public static function checkRuntimeDir() {
        umask(0);
        if(!is_dir(self::RUNTIME_DIR)) {
            if(@mkdir(self::RUNTIME_DIR, 0755) === false) {
                return false;
            }
        }

        if (self::$workerUserName) {
            $user_info = posix_getpwnam(self::$workerUserName);

            if (!chown(self::RUNTIME_DIR, $user_info['uid'])) {
                return false;
            }
            if (!chgrp(self::RUNTIME_DIR, $user_info['gid'])) {
                return false;
            }
        }

        if(!is_readable(self::RUNTIME_DIR) || !is_writeable(self::RUNTIME_DIR)) {
            return false;
        }
    ...
    public static function checkExtension()
    {
        ...
        foreach($need_map as $ext_name=>$must_required)
        {
            ...
            if(self::$eventLoopName == 'Select' && $suport)
            {
                ...
                {
                    // 禁用libevent，发现会有低概率接收超时问题 日期2014-12-23
