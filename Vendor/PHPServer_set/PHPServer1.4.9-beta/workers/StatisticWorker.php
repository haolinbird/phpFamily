...
class StatisticWorker extends PHPServerWorker
{
    ...
    // 多长时间写一次log数据
    protected $logSendTimeLong = 60;
    // 多长时间写一次统计数据
    protected $stSendTimeLong = 60;
    // 多长时间清除一次统计数据
    ...
    protected $qpsServiceUri = 'udp://10.17.46.36:3000';

    protected $failedQpsServiceUri = 'udp://10.17.46.36:3010';

    protected $logFailedServiceUri = 'udp://10.17.46.36:3020';
    ...
    public function dealProcess($recv_str)
    {
        ...
        {
            $this->statisticData[$module][$interface] = array('code'=>array(), 'suc_cost_time'=>0, 'fail_cost_time'=>0, 'suc_count'=>0, 'fail_count'=>0, 'time'=>$this->stLastWriteTime + $this->stSendTimeLong);
        ...
        {
            $this->statisticData[self::G_MODULE][self::G_INTERFACE] = array('code'=>array(), 'suc_cost_time'=>0, 'fail_cost_time'=>0, 'suc_count'=>0, 'fail_count'=>0, 'time'=>$this->stLastWriteTime + $this->stSendTimeLong);
    ...
    protected function wirteStToDisk()
    {
        ...
        foreach($this->statisticDataDetail as $module=>$items)
        {
        	...
        	foreach($items as $interface=>$data)
        	{
        		file_put_contents(SERVER_BASE."logs/statistic/detail/{$module}/{$interface}-detail|".date('Y-m-d',$this->stLastWriteTime + $this->stSendTimeLong - 1), ($this->stLastWriteTime + $this->stSendTimeLong)."\t".json_encode($data)."\n", FILE_APPEND | LOCK_EX);
        	}
        }
       ...
       foreach($this->statisticDataUserDetail as $module=>$items)
        {
        	...
        	foreach($items as $interface=>$data)
        	{
        		file_put_contents(SERVER_BASE."logs/statistic/userdetail/{$module}/{$interface}-detail|".date('Y-m-d',$this->stLastWriteTime + $this->stSendTimeLong - 1), ($this->stLastWriteTime + $this->stSendTimeLong)."\t".json_encode($data)."\n", FILE_APPEND | LOCK_EX);
        	}
        }
    ...
    protected function clearDisk($file = null, $exp_time = 86400)
    {
        ...
        $specialFile = strpos($file, "\\") !== false ? str_replace("\\", "\\\\", $file) : $file;

        $all_files = glob($specialFile."/*");
        if($all_files)
        {
