...
// require_once SERVER_BASE . 'Vendor/Bootstrap/Autoloader.php';
// require_once SERVER_BASE . 'Vendor/PHPClient/Text.php';
// require_once SERVER_BASE . 'Vendor/PHPClient/JMTextRpcClient.php';
// require_once SERVER_BASE . 'Vendor/PHPClient/JMRpcClient.php';
...
class TestClientWorker extends PHPServerWorker
{
    ...
    */
    ...
    public function dealProcess($recv_str)
    {
        ...
        if(!empty($_POST))
        {
            ...
            try{
                ...
                \PHPClient\Text::config($config);
                if (preg_match('/^[A-Za-z0-9]+_([A-Za-z0-9]+)_/', $class, $matches)) {
                    $module = $matches[1];
                ...
                $rsp_data = call_user_func_array(array(\PHPClient\Text::inst($module)->setClass($class), $func), $param);
            }catch(Exception $e){
                $rsp_data = $e.'';
            }
    ...
    protected function getServices($address)
    {
        ...
        static $autoloaderDirAdded = array();
        ...
        foreach (PHPServerConfig::get('workers') as $service_name=>$config)
        {
            ...
            $handler_dir = dirname($bootstrap).'/Handler'.DIRECTORY_SEPARATOR;
            $service_base_dir = dirname($bootstrap).DIRECTORY_SEPARATOR;
            if(!in_array($service_base_dir, $autoloaderDirAdded)){
                \Bootstrap\Autoloader::instance()->addRoot($service_base_dir);
                $autoloaderDirAdded[] = $service_base_dir; 
            }
    
            if(!empty($handler_dir))
            {
                $this->getServiceWithPath($handler_dir, $handler_dir);
            }
        }
        return $this->ServiceArray;
    }
    
    protected function getServiceWithPath($path = '', $baseDir)
    {
        foreach (glob($path.'/*') as $item) {
            if (is_dir($item)) {
                $this->getServiceWithPath($item, $baseDir);
                continue ;
            }
            require_once $item;
            
            /*
            $service_name = basename($item, '.php');
            */
            $service_name = str_replace(array($baseDir,'/','.php'), array('','\\',''), $item);
            $service_name = substr($service_name, 1, strlen($service_name));
    ...
    protected function display($req_text = '', $rsp_text = '', $rsp_data = '', $msg = '', $cost='')
    {
        ...
        $class = isset($_POST['class']) ? $_POST['class'] : '';
        $func = isset($_POST['func']) ? $_POST['func'] : '';
        $rsp_data = !is_scalar($rsp_data) || is_bool($rsp_data) || is_null($rsp_data) ? var_export($rsp_data, true) : $rsp_data;
