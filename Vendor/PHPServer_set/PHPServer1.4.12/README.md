...
发布日期: 2019-07-10    
特性:    
1. 兼容php7.1+的进程信号异步分派和中断处理机制.
 
### 版本: 1.4.11
发布日期: 2019-06-20    
特性:    
1. 支持通过设置环境变量PHPSERVER_CONTROL_CMD_PORT=10102,来指定phpserver控制命令的服务端口.
 

### 版本: 1.4.10
发布日期: 2019-04-8    
特性:    
1. 初始化服务注册和doveclient通讯时,增加一次重试,以防doveclient还没来得及启动.

### 版本: 1.4.9-beta
发布日期: 2019-02-12    
特性:    
1. 上报服务信息和保持心跳是，增加一次重试机会，避免依赖的服务或者环境偶尔不稳定时引起的误报.
2. 优化报警级别；改为向统一告警平台发送"
...
=======
- PHPServer支持telnet远程控制及监控,默认是10101端口,可通过设置环境变量(推荐)PHPSERVER_CONTROL_CMD_PORT或者配置中添加control_cmd_port进行指定.
- 使用方法
<pre>
telnet ip 10101 
输入密码P@ssword
