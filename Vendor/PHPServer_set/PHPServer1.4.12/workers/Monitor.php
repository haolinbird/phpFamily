...
class Monitor extends PHPServerWorker
{
    ...
    // 多长时间检查一次业务成功率
    const CHECK_BUSINESS_TIME_LONG = 60;
    // 接收telnet数据超时时间 ms
    ...
    public function checkBusiness() {
        ...
        foreach($module_interface_map as $module=>$interfaces) {
            ...
            foreach($interfaces as $interface) {
                ...
                $MIN_BUSINESS_lOW_SUCCESS_RATE =
                    PHPServerConfig::get('workers.Monitor.framework.min_success_rate') > 0 ? PHPServerConfig::get('workers.Monitor.framework.min_success_rate') : self::MIN_BUSINESS_lOW_SUCCESS_RATE;
    ...
    public function showStatus()
    {
        ...
        $address .= $port;
        $this->telnetSocketToMaster = stream_socket_client("tcp://{$address}", $no, $msg);
    ...
    protected function onMasterDead()
    {
        ...
        $this->sendUnifiedAlarm($alarm_data, '告警消息 PHPServer框架监控 ip:'.$ip.' 主进程意外退出 时间：'.date('Y-m-d H:i:s'));
    }
    ...
    public function onTooManyWorkersExits($worker_name, $status, $exit_count)
    {
        ...
        if(65280 == $status || 30720 == $status)
        {
            ...
            $this->sendUnifiedAlarm($alarm_data, '告警消息 PHPServer框架监控 '.$ip.' '.$worker_name.' '.((self::CHECK_BUSINESS_TIME_LONG/60)).'分钟内出现 FatalError '.$exit_count.'次 时间:'.date('Y-m-d H:i:s'));
        }
        elseif(9 == $status)
        {
        	$alarm_data = array(
        	...
        	$this->sendUnifiedAlarm($alarm_data, '告警消息 PHPServer框架监控 '.$ip.' '.$worker_name.' 业务进程阻塞，阻塞进程数：'.$exit_count.' 时间:'.date('Y-m-d H:i:s'));
        }
        else
        {
        	$alarm_data = array(
            ...
            $this->sendUnifiedAlarm($alarm_data, '告警消息 PHPServer框架监控 '.$ip.' '.$worker_name.' 进程频繁退出 退出次数'.$exit_count.' 退出状态码：'.$status .' 时间:'.date('Y-m-d H:i:s'));
        }
    ...
    public function onFrameworkLowSuccessRate($worker_name, $success_rate)
    {
        ...
        $this->sendUnifiedAlarm($alarm_data, '告警消息 PHPServer框架监控 '.$ip.' '.$worker_name.' 成功率:'.$success_rate.'% 时间：'.date('Y-m-d H:i:s'));
    ...
    public function onBusinessLowSuccessRate($module, $interface, $total_count, $fail_count, $success_rate, $time)
    {
        ...
        $this->sendUnifiedAlarm($alarm_data, "告警消息 PHPServer业务监控 请求{$module}::{$interface} {$total_count}次，失败{$fail_count}次，成功率{$success_rate}% ip:{$ip} 时间:" . date('Y-m-d H:i:s', $time));
    ...
    protected function sendAlarm($data, $content = null)
    {
        ...
        if ($use_unified_alarm) {
            return $this->sendUnifiedAlarm($data, $content);
        }
    ...
    protected function sendUnifiedAlarm($data, $content) {
	    ...
	    $this->notice("send unified alarm :$content. return: $res");
        return true;
    }
    ...
    public function clearLogs($dir)
    {
        clearstatcache();
        $time_now = time();
        foreach($this->glob($dir."/20*-*-*") as $file)
        {
    ...
    {
        return is_file($path) ? unlink($path) : array_map(array($this, 'recursiveDelete'),$this->glob($path.'/*')) == rmdir($path);
    }
