...
class HTTP{
    ...
    public static function input($data)
    {
        // 检查客户端数据是否已经读完.
        // POST请求还要读包体
        if(false !== strpos($data, "POST"))
        {
            // 看包体长度是否符合
            $tmp = explode("\r\n", $data);
            $content_lenght = $body_part = null;
            foreach($tmp as $line => $d){
                if(strpos($d, 'Content-Length:') === 0){
                    $content_lenght = (int)substr($d, 15);
                } else {
                    if($d === '' && isset($tmp[$line + 1]) ){
                        $body_part = $tmp[$line + 1];
                    }
                }
                if($content_lenght !== null && $body_part !== null){
                    break;
                }
            }
            if($content_lenght < 1){
                return 0;
            } else if($body_part === null || strlen($body_part) >= $content_lenght){
                return 0;
            } else {
                return 1;
            }
    ...
    public static function decode($data)
    {
        ...
        foreach($header_data as $content)
        {
            ...
            elseif(strpos($content, 'If-Modified-Since') === 0)
            {
                list($key, $If_Modified_Since) = explode(":", $content);
                $_SERVER['HTTP_IF_MODIFIED_SINCE'] = $If_Modified_Since;
            }
        }
