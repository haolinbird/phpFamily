...
class ThriftWorker extends PHPServerWorker
{
    ...
    public function dealProcess($recv_str)
    {
        ...
        if(strpos($recv_str, "\nRPC") === 1 || strpos($recv_str, "\nTEST") === 1 || strpos($recv_str, "\nPING") === 1) {
            ...
            $this->dealTextProcess($recv_str);
        }else { // thrift协议
        	$this->currentProtocol = 'thrift';
            $this->dealThriftProcess($recv_str);
        }
    }
    ...
    public function dealTextProcess($recv_str)
    {
        try {
            ...
            if (!empty($data['proxy_version'])) {
                $this->fdClientVersion[$this->currentDealFd] = self::CLIENT_PROXY;
            } else {
                $this->fdClientVersion[$this->currentDealFd] = self::CLIENT_PHPCLIENT;
            }
    ...
    protected function process($data)
    {
        ...
        catch (Exception $ex)
        {
            ...
            if(isset($call_back) && method_exists($call_back[0], 'onException')) {
                $call_back[0]->onException($ex);
            }
        }
        ...
        {
            // 这里一般是关闭数据库链接等操作
            \on_phpserver_request_finish(__CLASS__, $data, $ctx);
        }
    ...
    protected function send($data)
    {
        ...
        // 如果是proxy发过来的请求，为了解决长连接存在的一些问题，需要对协议做修正。
        if (@$this->fdClientVersion[$this->currentDealFd] === self::CLIENT_PROXY) {
            // 如果本worker已经决定停止了，那么应该告诉proxy不要再发请求过来。
            $flag = self::KEEP_RPOXY_CONNECTION;
            if ($this->workerStatus === self::STATUS_SHUTDOWN) {
                $flag = self::RELEASE_RPOXY_CONNECTION;
            }
            $encode_text = sprintf("%d\n%s\n%d\n%s\n%s", strlen(self::VERSION), self::VERSION, strlen($flag), $flag, $encode_text);
        }
    ...
    public function dealThriftProcess($recv_str) {
        ...
        $useConnectPool = \Thrift\Context::get('use_connect_pool');
        if(!empty($useConnectPool)) {
            $this->fdClientVersion[$this->currentDealFd] = self::CLIENT_PROXY;
        } else {
            $this->fdClientVersion[$this->currentDealFd] = self::CLIENT_PHPCLIENT;
        }
        ...
        else
      {
        ...
        try {
            ...
            $stdProcessorClass = $this->providerNamespace.'\\'.$serviceName.'Processor';
            $serviceNamePath = str_replace('\\', '/', $serviceName);
            if(!class_exists($processorClass, false))
            {
                $types_file = $this->providerDir.'/'.$serviceNamePath.'/Types.php';
                if(!is_file($types_file))
                {// 尝试标准命名空间的支持.
                    if(!class_exists($stdProcessorClass, false)){
                        $types_file = dirname($this->providerDir.'/'.$serviceNamePath).'/Types.php';
                        if(!is_file($types_file)){
                            throw new \Exception('None of class ' . $processorClass  . ' and '.$stdProcessorClass.' found', 405);
                        }
                        require_once $types_file;
                        require_once $this->providerDir.'/'.$serviceNamePath.'.php';
                    }
                    $processorClass = $stdProcessorClass;
                } else {
                    require_once $types_file;
	   ...
	   // 文件不存在尝试从磁盘上读取
            ...
            {
                clearstatcache();
                $handler_file = $this->handlerNamespace == 'Provider' ? /*不支持命名空间 */ $this->handlerDir.'/'.$serviceName.'/'.$serviceName.'Handler.php' : $this->handlerDir.'/'.$serviceNamePath.'.php';
                if(is_file($handler_file))
                {
                    require_once $handler_file;
                }
        ...
        try {
            ...
            if ($process_used_time >= $this->slowlogTime) {
                $slowlog_data = array(
                    ...
                    'method'                => $method_name,
                    'source_ip'             => $source_ip,
                    'request'               => self::$currentRequestBuffer,
                    'process_used_time_ms'  => $process_used_time,

            );
                \MNLogger\SlowLogger::instance()->log(json_encode($slowlog_data));
            }
    ...
    protected function addContext() {
        // 如果是proxy发过来的请求，为了解决长连接存在的一些问题，需要对协议做修正。
        if (@$this->fdClientVersion[$this->currentDealFd] === self::CLIENT_PROXY) {
            // 如果本worker已经决定停止了，那么应该告诉客户端(proxy)不要再发请求过来。
            $flag = self::KEEP_RPOXY_CONNECTION;
            if ($this->workerStatus === self::STATUS_SHUTDOWN) {
                $flag = self::RELEASE_RPOXY_CONNECTION;
            }
            ...
            \Thrift\Context::put('proxy_context', json_encode($proxy_context));
        }
