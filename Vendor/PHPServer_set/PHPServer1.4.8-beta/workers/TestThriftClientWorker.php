...
// 由于业务的bootstrap先执行，所以会先使用业务的Vendor目录，这里切记不能改变加载顺序
...
HHH;
    ...
    public function getService()
    {
        ...
        foreach (PHPServerConfig::get('workers') as $worker_name=>$config)
        {
            ...
            $cmd = SERVER_BASE.'scripts/traverse_handlers.php '.escapeshellarg($bootstrap).' '.escapeshellarg($provider_dir).' '.escapeshellarg($handler_dir).' '.$address;
            $result = exec($cmd, $output, $return);
            $result = json_decode($result, true);
            if(!$result){
                continue;
            }
            $client_config = array_merge($client_config, $result['config']);
            $this->thriftServiceArray = array_merge($this->thriftServiceArray, $result['services']);
        }
...
class ClientForTest
{
     ...
     * @param array $config
     */
    public static function configAlarm($config)
    {
        self::$smsAlarmPrarm = $config;
    }
    ...
    public static function sendSmsAlarm($content)
    {
        ...
        $phone_array = explode(',', self::$smsAlarmPrarm['phone']);
