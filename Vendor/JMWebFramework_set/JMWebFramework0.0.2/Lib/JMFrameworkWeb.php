<?php
require_once(__DIR__.DIRECTORY_SEPARATOR . 'JMFramework.php');
require_once __DIR__.DIRECTORY_SEPARATOR . 'JMViewController.php';
...
class JMSiteEngine
{
    ...
    protected $routePath = '/';  //routePath 只负责原样记录 url routing 传入的 _rp_ （可能是空）
    ...
    public function ensureMainSiteAndLearnSubDomain()
    {
        if (isset($_SERVER['HTTP_HOST'])) {
            ...
            if ($domainNameFieldsNumber <= 2) {
                ...
                JMSystem::RedirectExit("http://{$host}{$_SERVER["REQUEST_URI"]}", 301);
            }
            else
            {
    ...
    public function run()
    {
        ...
        if(JMClassAutoLoader::Load($testControllerClassName) )
        ...
        //we get valid routePath and routePathFields now.
        ...
        else //for normal page
        {
            ...
            if(!is_null($viewVars))
            {
                $this->viewController->displayActionTemplate($viewVars);
            }
        } // end if (ajax/normal)
        ...
        if(!is_null($ajaxData))
        {
            $this->viewController->displayJson($ajaxData, JMSystem::GetGet('callback'));
        }
    } // end if function run

    public static function validateJsonpCallback()
    {
        $callback = JMSystem::GetGet('callback');
    ...
    public function defaultBadJsonpPage()
    {
...
class JMHtml
{
    ...
    public function formBegin($url = '', $htmlOptions = array())
    {
        ...
        if(empty($htmlOptions['method'])) $htmlOptions['method'] = 'post';
...
class JMDataFormCheck
{
    ...
    {
        $this->dataForm->addMessageIfError($msg);  //chain to the JMDataForm
        return $this;
    }
    ...
    {
        $this->result = $this->result && JMUtility::IsValidEMail($this->value);
        return $this;
    }
    public function isValidDate()
    {
        $this->result = $this->result && JMUtility::IsValidDate($this->value);
        return $this;
    }
    public function isValidDatetime()
    {
        $this->result = $this->result && JMUtility::IsValidDatetime($this->value);
        return $this;
    }
    public function isValidCodeSymbol()
    {
        $this->result = $this->result && JMUtility::IsValidCodeSymbol($this->value);
        return $this;
    }
    public function isBetween($a, $b)
    {
        $ret = true;
        if( $a !== null) $ret = $ret && ($a <= $this->value);
        if( $b !== null) $ret = $ret && ($this->value <= $b);
    ...
    public function isLengthBetween($a, $b)
    {
        $l = strlen($this->value);
        $ret = true;
        if( $a !== null) $ret = $ret && ($a <= $l);
        if( $b !== null) $ret = $ret && ($l <= $b);
        $this->result = $this->result && $ret;
        return $this;
    }
...
class JMDataForm
{
    ...
    protected function initFromInput($inputs, $keys, $name)
    {
        ...
        foreach($keys as $k=>$def)
        {
            ...
            if(isset($inputs[$k]))
            {
                ...
                else if(is_array($def)) //TODO: merge with default
    ...
    public function check($k)
    {
        ...
        $this->currentChecking = new JMDataFormCheck($this, $k, $this->get($k));
...
class JMTipMessageManager
{
    ...
    {
        if( ! self::$default)
            self::$default = new JMTipMessageManager();
        return self::$default;
    }
}
