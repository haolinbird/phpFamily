...
require_once(JM_WEB_FRAMEWORK_ROOT. 'JMFrameworkWeb.php');

class JMViewController_WebManagementBase extends JMViewController
{
    ...
    public function initialize()
    {
        ...
        $this->siteInfo['RemoteIp'] = JMSystem::GetRemoteIp();
        $this->siteInfo['RemoteIpWithProxy'] = JMSystem::GetRemoteIpWithProxy();
        ...
        JMRegistry::set('SiteInfo', $siteInfo);
    }
    ...
    public function __get($name)
    {
        switch($name)
        {
            ...
            default:
                ...
                {
                    trigger_error(new \Exception('Accessing undefined property: '.get_called_class().'::'.$name.' via magic __get.'), E_USER_NOTICE);
                }
        }
    }
    ...
    {
        $config = JMRegistry::get('SiteInfo');
        ...
        if($data)
        {
            if( ! is_array($data)) throw new InvalidArgumentException('data must be an array for smarty assign');
...
class JMAuthLdapSignon extends JMAuthInterface
{
    const SESSION_KEY_AUTH_INFO = 'JMAuthLdapSignon_auth_info';
    ...
    public function ensureLogin()
    {
        ...
        if($this->siteEngine->getRoutePath() == "$defaultRoutePathBaseName/AuthSignonLoginCallback")
        {
            if($this->processAuthSignonLoginCallback())
                JMSystem::RedirectExit(JMGetRequest('redirect_url', '/'), 302);
        ...
        else if($this->siteEngine->getRoutePath() == "$defaultRoutePathBaseName/AuthSignonLogout")
        {
            $this->logout('/');
        }
        else if( empty ($_SESSION[self::SESSION_KEY_AUTH_INFO]))
        {
            ...
            JMSystem::RedirectExit("{$this->signonConfig['BaseUrl']}auth/login?camefrom=" . rawurlencode($this->signonConfig['AppName']) . '&login_url=' . rawurlencode($login_url), 302);
        }
    ...
    public function logout($gotoUrl = '/')
    {
        ...
        if(empty($gotoUrl)) trigger_error ('unknown logout goto url');

        $protocol = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] == 'on') ? 'https' : 'http';
        if($_SERVER['SERVER_PORT'] == 443) $protocol = 'https';
        if(strpos($gotoUrl, '://') === false) $gotoUrl = "$protocol://{$_SERVER['HTTP_HOST']}$gotoUrl";

        JMSystem::RedirectExit("{$this->signonConfig['BaseUrl']}auth/logout?camefrom=" . rawurlencode($this->signonConfig['AppName']) . '&goto_url=' . rawurlencode($gotoUrl), 302);
    }
}
