...
class JMTextRpcClient {
    ...
    public function sendTLV($buffer, $conn) {
        ...
        try {
            $larkBuffer = \protocol\Lark::encode(\protocol\Lark::TAG_RPC_CLIENT_SEND, array($buffer, $this->config['service']));
    ...
    public function recvTLV($conn) {
        ...
        while (($leftLen = \protocol\Lark::input($buffer)) > 0) {
            ...
            if (!$tmp && $tmp !== '0') { // 字符串0是一个神奇的存在，bool值为false
                ...
                throw new \Exception(sprintf('[Lark协议]读取失败原因:%s。服务名[%s]',$err, $this->config['service']));
            }
            $buffer .= $tmp;
        }
        ...
        if ($tag !== \protocol\Lark::TAG_RPC_CLIENT_RECV) {
            throw new \Exception(sprintf('请求异常，错误的Tag[%d],期望Tag[%d]:%s', $tag, \protocol\Lark::TAG_RPC_CLIENT_RECV, $larkData['data'][0]));
        }
