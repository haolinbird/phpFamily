...
class JMDbKeyBatch {
     ...
     * @throws Exception
     */
    private function getIDCNumbers() {
        $IDCNumbers = \Config\GlobalKey::$IDCNumbers;

        if (!is_array($IDCNumbers)) {
            throw new Exception('IDCNumbers must be array');
        }

        foreach ($IDCNumbers as $number) {
            if ($number >= static::PART_COUNT) {
                throw new Exception('idc number must less than ' . static::PART_COUNT);
            }
        }
    ...
    public function getId($projectFlag) {
        ...
        $IPC_KEY = crc32($projectFlag . 'OrderedPart' . '20161101133600') | $IPC_KEY;
...
//        $partNumber = 0;
        ...
        //$IPC_KEY = 0x1234; // System V IPC KEY
        $IPC_KEY = ftok(__FILE__, 'R'); //因为这个IPC_KEY需要为整型，目前的方案选择取ftok，或者用整型标识各个项目
        $IPC_KEY = crc32($projectFlag . 'RandomPart' . '20161101133600') | $IPC_KEY;
        $SEQ_KEY = 8; // 共享内存中存储序列号ID的KEY  int类型
        // 创建或获得一个现有的，以"1234"为KEY的信号量
        $sem_id = sem_get($IPC_KEY);
        // 创建或关联一个现有的，以"1234"为KEY的共享内存
        ...
        if (is_array($idArr) && count($idArr)) {
            ...
            shm_put_var($shm_id, $SEQ_KEY, $idArr);
            // 释放信号量，相当于解锁
            sem_release($sem_id);
            // 关闭共享内存关联
            shm_detach($shm_id);
            ...
            shm_put_var($shm_id, $SEQ_KEY, $putArr);
            // 释放信号量，相当于解锁
            sem_release($sem_id);
            // 关闭共享内存关联
            shm_detach($shm_id);
