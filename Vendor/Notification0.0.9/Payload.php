...
class Payload
{
    ...
    public function setProperties($k, $v)
    {
        $p = $this->get('properties');
        if (is_array($p)) {
            $p[$k] = $v;
        } else {
            $p = array(
    ...
    {
        return isset($this->param[$k]) ? $this->param[$k] : null;
    }

    public function set($k, $v)
    {
        $this->param[$k] = (string)$v;
        return $this;
    }

    public function __toString()
    {
        return http_build_query($this->param, null, '&');
    }

    public function checkLong()
    {
        //check apns length
        $n = $this->getApnsLength();
        if ($n > self::ApnsPayloadLength) {
            $ex = new PayloadException('payload is too long');
            $ex->setExcessLength($n - self::ApnsPayloadLength);
            throw $ex;
        }
    }

    public function sign($key, $secret)
    {
        $time = $time = explode(" ", microtime());
        $time = $time [1] . ($time [0] * 100000000);
        $this->set('__time__', $time);
        $this->set('__key__', $key);
        $arr = array_values($this->param);
        array_push($arr, $secret);
        sort($arr, SORT_STRING);
        $tmp = implode("", $arr);
        $this->param['__sign__'] = md5($tmp);
    }

    public function getApnsLength()
    {

        $payload['aps'] = array();

        if (isset($this->param['text'])) {
            $payload['aps']['alert'] = (string)$this->param['text'];
        }
        if (isset($this->param['badge']) && $this->param['badge'] > 0) {
            $payload['aps']['badge'] = (int)$this->param['badge'];
        }
        if (isset($this->param['sound'])) {
            $payload['aps']['sound'] = (string)$this->param['sound'];
        }

        if (isset($this->param['properties']) && is_array($this->param['properties'])) {
            foreach ($this->param['properties'] as $sPropertyName => $mPropertyValue) {
                $payload[$sPropertyName] = $mPropertyValue;
            }
        }

        //php 5.4 以上
        if (version_compare(phpversion(), '5.4', '>=')) {
            $json = json_encode($payload, JSON_UNESCAPED_UNICODE | JSON_FORCE_OBJECT);
        } else {
            $json = jsonlite_encode($payload);
        }
...
class PayloadException extends \Exception
{
    ...
    protected $excessLength;

    public function getExcessLength()
    {
        return $this->excessLength;
    }

    public function setExcessLength($l)
    {
        $this->excessLength = $l;
    }
