        ...
        for ($i = 0; $i < $retry; $i++) {
            try{
                ...
                if ($this->useConnectPoolFlag == false) {
                    throw new Exception('RPC Exception: '.$class.'::'.$method. ' '.$ex->getMessage());
                }
            }
        }
    }
    ...
    public function __call($method, $arguments)
    {
        ...
        for ($i = 0; $i < $retry; $i++) {
            try {
                ...
                if ($this->rpcClient->getConnnectPoolStatus() == false) {
                    throw new Exception('RPC Exception: '.$this->class.'::'.$method. ' '.$ex->getMessage());
                }
            }
        }
    }
