...
class Lark
{
    ...
    protected static $valueItemCountMap = array(
        self::TAG_RPC_CLIENT_SEND       => 3,
        self::TAG_RPC_CLIENT_RECV       => 2,
        self::TAG_RPC_LARK_WRITE_ERR    => 2,
        self::TAG_RPC_LARK_READ_ERR     => 2,
    ...
    protected static function getValueItem($buffer, $itemCount = 1) {
        ...
        for ($i = 0; $i < $itemCount; $i++) {
            $binLen = substr($buffer, $offset, 4);
            if (empty($binLen)) { // 可能PHP这边先升级，对方还是老版本，所以传过来的字段数不够,忽略就好
                break;
            }
            $unpackData = unpack('Nlen', $binLen);
            $len = $unpackData['len'];
            $offset += 4;
