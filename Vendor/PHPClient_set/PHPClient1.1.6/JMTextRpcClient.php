 ...
 * 发布日期：2014-11-30
 * 特性更新：
 * 2014-11-30 修复checkConfigMd5 notice
...
class JMTextRpcClient
{
    ...
    public static function getBadAddressList($use_cache = true)
    {
    	...
    	{
    		// 是否有故障节点
    		if(!shm_has_var(self::getShmFd(), self::RPC_BAD_ADDRESS_KEY))
    		{
    			self::$badAddressList = array();
    		}
    		else
    ...
    public static function sendSmsAlarm($content)
    {
    	...
    	// 短信告警
    	if(empty(self::$smsAlarmPrarm['phone']) && isset(\Config\PHPClient::$smsAlarmPrarm))
    	{
    ...
    public static function getLastAlarmTime()
    {
    	...
    	// 是否有保存上次告警时间
    	if(!shm_has_var(self::getShmFd(), self::RPC_LAST_ALARM_TIME_KEY))
    	{
    		$time_now = time();
    		self::setLastAlarmTime($time_now);
    		return $time_now;
    	}
    	return shm_get_var(self::getShmFd(), self::RPC_LAST_ALARM_TIME_KEY);
    }
