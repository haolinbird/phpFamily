...
class JMTextRpcClient
{
    ...
    {
        $r = $this->dealCtx($this->recvData($conn));
        $this->endTingYunWebService();
        return $r;
    }
    ...
    *@return array   待发送数据
    */
    public function initRpcData($class, $method, $arguments, $traceStr)
    {
        $packet = array(
            'data' => json_encode(
                array(
    ...
    public function __call($method, $arguments)
    {
        ...
        $traceStr = $this->startTingYunWebService($url);

        $packet = $this->initRpcData($class, $method, $arguments, $traceStr);
    ...
    */
     ...
     * @return boolean|null
     */
    public function startTingYunAction($class, $method, $traceStr = null)
    {
        if (!$this->isTingYunAgentOn()) {
            return false;
        }
        $action = $class . ':' . $method;
        if (function_exists('tingyun_start_webaction')) {
            return \tingyun_start_webaction($action, $traceStr);
        }
    }
     ...
     * @return boolean|null
     */
    public function endTingYunAction()
    {
        if (!$this->isTingYunAgentOn()) {
            return false;
        }
        if (function_exists('tingyun_end_webaction')) {
            return \tingyun_end_webaction();
        }
    }
     ...
     * @return void
     */
    public function noticeTingYunError(\Exception $e)
    {
        if (!$this->isTingYunAgentOn()) {
            return;
        }
     ...
     * 获得听云的应用跟踪字符串.
     *
     * @param string $url webservice的url全路径，包括协议.
     *
     * @return string|null
     */
    public function startTingYunWebService($url)
    {
        if (!$this->isTingYunAgentOn()) {
            return null;
        }

        if (function_exists('tingyun_start_webservice')) {
            return \tingyun_start_webservice($url);
        }
    }
     ...
     * @return string|null
     */
    public function endTingYunWebService()
    {
        if (!$this->isTingYunAgentOn()) {
            return null;
        }

        if (function_exists('tingyun_end_webservice')) {
            return \tingyun_end_webservice();
        }
    }
     ...
     * @return boolean
     */
    public function isTingYunAgentOn()
    {
        return \Config\Rpc::$tingyunAgentOn;
    }
    ...
    public function getUrl($method)
    {
        ...
        $this->formatAddress($this->rpcUri, $uri, $weight);
        return 'JMTextRpc://' . $uri . '/' . $class . '/' . $method;
    }
...
class ServiceClassWrapper{
    ...
    public function __call($method, $arguments)
    {
        ...
        $url = $this->rpcClient->getUrl($method);

        $traceStr = $this->rpcClient->startTingYunWebService($url);
        /* 处理听云相关字符串*/

        $packet = $this->rpcClient->initRpcData($this->class, $method, $arguments, $traceStr);
