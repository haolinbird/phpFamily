...
class Text extends JMTextRpcClient{
    ...
    public static function inst($config)
    {
        if(is_array($config))
        {
            ...
            {
                parent::config(array_merge($allConfig, array($configName => $config)));
            }
        }
        else
      {
    ...
    protected function __construct($configName)
    {
        ...
        if(!self::$hasLoadConfig && class_exists('\Config\PHPClient'))
        {
            ...
            $cfgObject = new \Config\PHPClient;
            // if (property_exists($cfgObject, $configName) && (! isset($cfgObject->{$configName}['service']) || ! $cfgObject->{$configName}['service'])) {
            //     $cfgObject->{$configName}['service'] = $configName;
            // }

            $config = parent::config((array) $cfgObject);
        }
