...
class JMTextRpcClient
{
    ...
    /**
     * 客户端接收超时设置 单位秒(特定项目上的具体配置,没有则使用公共配置).
     * @var int
     */
    protected $recvTimeOutByProject = 0;

    // 如果用户设定超时为3秒, 传递给phpserver/mcp的超时还是3s, 但是实际phpclient的超时时间要增加该参数设置的毫秒,为接收因为超时导致的错误留出余量.
    public static $recvTimeOutOffset = 50;
    ...
    // 是否尝试使用连接池.
    protected $isUseConnectPool = false;
    // 当前是否正在使用连接池.
    protected $useConnectPoolFlag = false;
    protected $mcpServer = '';
    // 服务名到机器列表的映射.
    protected static $serviceToHosts = array();
    // mcpd生成 服务->机器列表 文件所在的目录.
    public static $serviceLogDir = '/var/lib/mcpd/service_snapshot/';
    ...
    public function getConnectPoolCfgStatus()
    {
        return $this->isUseConnectPool;
    }

    public function getRpcService()
    {
        return $this->rpcService;
    }

    public function setRpcUri($uri)
    {
        $this->rpcUri = $uri;
    }
    ...
    protected function __construct(array $config = array())
    {
        ...
        if(!empty($config['recv_time_out_offset']) && (int)$config['recv_time_out_offset'] > 0)
        {
            self::$recvTimeOutOffset = (int)$config['recv_time_out_offset'];
        }

        // mcpd存放 服务->机器 关系的目录.
        if (isset($config['service_log_dir']) && ! empty($config['service_log_dir'])) {
            static::$serviceLogDir = $config['service_log_dir'];
        }
        ...
        if (preg_match('/^[A-Za-z0-9]+_([A-Za-z0-9]+)/', $className, $matches)) {
            ...
            if (empty($config[$module])) {
                ...
                if (! isset($config[$module]['service']) || empty($config[$module]['service'])) {
                    $config[$module]['service'] = $module;
                }
    ...
    public function init(array $config)
    {
        ...
        // mcpd生成 服务->机器列表 文件所在的目录.
        if (isset($config['recv_time_out']) && $config['recv_time_out'] > 0) {
            $this->recvTimeOutByProject = $config['recv_time_out'];
        } else {
            $this->recvTimeOutByProject = self::$recvTimeOut;
        }

        if (isset($config['service'])) {
            $this->rpcService = $config['service'];
        }

        if (isset($config['dove_key'])) {
            $this->doveKey = $config['dove_key'];
        }

        if (defined('USE_CONNECT_POOL') && USE_CONNECT_POOL == true) {
            if (defined('MCP_SERVER') && MCP_SERVER != '') {
                $this->mcpServer = MCP_SERVER;
            } else {
                $proxyHost = ini_get('redis.proxy.host');
                $proxyPort = ini_get('redis.proxy.port');

                if ($proxyHost && (stripos($proxyHost, 'unix://') === 0 || strpos($proxyHost, '/') === 0)) {
                    $this->mcpServer = $proxyHost;
                } else if ($proxyHost && $proxyPort > 0) {
                    $this->mcpServer = "{$proxyHost}:{$proxyPort}";
                }
            }

            if ($this->mcpServer == '') {
                $this->isUseConnectPool = false;
            } else {
                $this->isUseConnectPool = true;

                if (substr($this->mcpServer, 0, 1) == '/') {
                    $this->mcpServer = 'unix://' . $this->mcpServer;
                }

                if (empty($this->rpcService) && empty($this->doveKey)) {
                    throw new \Exception('"service" and "dove_key" must be at least one is not empty');
                }
            }
        }

        // 如果不走连接池，则uri必须配置.
        if ($this->isUseConnectPool == false && empty($this->rpcUri)) {
            if (empty($this->rpcService)) {
                throw new \Exception('Uri cannot be empty');
            }

            // 在mcpd维护的文件中找特定服务的机器列表.
            $this->rpcUri = $this->getHostByService($this->rpcService);
            if (empty($this->rpcUri)) {
                throw new \Exception('Failed to get service list(' . $this->rpcService . '), Uri cannot be empty.');
            }
        }
    }
     ...
     * @return string
     */
    public function getHostByService($service)
    {
        if (! empty($this->rpcUri)) {
            return $this->rpcUri;
        }
        ...
        $serviceLogFile = static::$serviceLogDir . "/{$this->rpcService}";
        if (! file_exists($serviceLogFile)) {
            return '';
        }

        if (! isset(static::$serviceToHosts[$this->rpcService]) || static::$serviceToHosts[$this->rpcService]['mtime'] != filemtime($serviceLogFile)) {
            static::$serviceToHosts[$this->rpcService] = array(
                'mtime' => filemtime($serviceLogFile),
                'uri' => explode(',', trim(file_get_contents($serviceLogFile))),
            );
        }

        if (! isset(static::$serviceToHosts[$this->rpcService]) || empty(static::$serviceToHosts[$this->rpcService]['uri'])) {
            return '';
        }

        $config = $this->filter(array('uri' => static::$serviceToHosts[$this->rpcService]['uri']));
        return $config['uri'];
    }
     ...
     * 创建网络链接.
     *
     * @param boolean $ignoreConnectPool 是否使用连接池.
     *
     * @throws Exception 抛出链接错误信息.
     *
     * @return Socket connection resource.
     */
    public function openConnection($useConnectPool = true)
    {
        $this->executionTimeStart = microtime(true);

        // 尝试使用连接池.
        if ($this->isUseConnectPool == true && $useConnectPool == true) {
            $conn = @stream_socket_client($this->mcpServer, $errno, $errstr, static::CONN_TRY_TIMEOUT);
            if (! $conn && $errno == 110) {
                $timeout = (int)(self::$connectionTimeOut - static::CONN_TRY_TIMEOUT);
                if ($timeout > 0) {
                    $conn = stream_socket_client($this->mcpServer, $errno, $errstr, $timeout);
                }
            }

            // 只有连不上mcp时才考虑重试,防止重复给业务方发送数据.
            if (! $conn) {
                throw new McpException($errstr, $errno);
            }

            stream_set_timeout($conn, $this->recvTimeOutByProject, self::$recvTimeOutOffset * 1000);

            $head = sprintf("jmtext://servicename=%s;dovekey=%s;read_remote_conn_timeout=%s", $this->rpcService, $this->doveKey, $this->recvTimeOutByProject);
            $buf = sprintf("%d\n%s\n", strlen($head), $head);
            if (fwrite($conn, $buf) != strlen($buf)) {
                throw new Exception(sprintf('JMTextRpcClient: %s, %s (%.3fs)', $this->mcpServer, 'Failed to write data', $this->executionTime()));
            } else {
                $response = fgets($conn);
                if ($response{0} != '+') {
                    throw new Exception(sprintf('JMTextRpcClient: %s, %s (%.3fs)', $this->mcpServer, $response, $this->executionTime()));
                }
            }
    ...
    */
    public function initRpcData($class, $method, $arguments)
    {
        ...
        // 当前正在使用连接池.
        if ($this->useConnectPoolFlag == true) {
            $packet['data']['proxy_version'] = 'v1.0';
            $packet['data']['read_remote_conn_timeout'] = $this->recvTimeOutByProject;
        }

        $packet['data'] = json_encode($packet['data']);
    ...
    public function __call($method, $arguments)
    {
        ...
        $retry = $this->isUseConnectPool == true ? 2 : 1;
        for ($i = 0; $i < $retry; $i++) {
            try{
                // 如果第一次请求不成功，后续的请求会忽略连接池，进行直连.
                $conn = $this->openConnection($i == 0 ? true : false);
                ...
                if (empty($this->rpcUri)) {
                    ...
                    $this->rpcUri = $this->getHostByService($this->rpcService);
                    if (empty($this->rpcUri)) {
                        throw new \Exception('Failed to get service list(' . $this->rpcService . ')');
                    }
                }
            }
        }
    ...
    */
    protected function dealCtx($ctx)
    {
        ...
        if (is_array($ctx) && isset($ctx['exception']) && is_array($ctx['exception'])) {
            ...
            if ($this->useConnectPoolFlag == true) {
                throw new Exception('URI：' . $this->mcpServer . ' return ' . var_export($ctx['exception'], true));
            }

            throw new Exception('URI：' . $this->rpcUri . ' return ' . var_export($ctx['exception'], true));
        }
        elseif(is_array($ctx) && isset($ctx['rpc_business_exception']))
        {// 服务端报告有错误。
            $trace_logger && $trace_logger->RPC_CR('EXCEPTION', strlen(json_encode($ctx)), $ctx);
        }
        else 
    ...
    */
    protected function sendData(array $data, $conn)
    {
        ...
        /*if ($this->isUseConnectPool == true) {
            $context['service_provider_name'] = $this->rpcService;
            $context['dovekey'] = $this->doveKey;
        }*/
        ...
        if ($len != strlen($buffer)) {
            if ($this->useConnectPoolFlag == true) {
                throw new Exception("JMTextRpcClient: Write $total_length bytes to " . $this->mcpServer . " fail (".$this->executionTime()."s). fwrite return " . var_export($send_length, true));
            }
    ...
    protected function recvData($conn)
    {
        $rpcUri = $this->rpcUri;
        if ($this->useConnectPoolFlag == true) {
            $rpcUri = $this->mcpServer;
        }
        // 读取 RPC 返回数据的长度信息
        if (!$length = fgets($conn)) {
            $execution_time = $this->executionTime();
            if($execution_time>=$this->recvTimeOutByProject)
            {
                throw new Exception(
                    sprintf(
        ...
        while (strlen($ctx) < $length) {
            ...
            if(false !== $buf){
                ...
                switch(true){
                    ...
                    case $meta = stream_get_meta_data($conn) && $meta['timed_out'] :
                        throw new Exception(
                            sprintf(
                                'JMTextRpcClient: Network %s recv timeout (%.3fs), read [%d/%d]bytes',
                                $rpcUri,
                                $execution_time,
                                strlen($ctx),
                                $length
                    ...
                    default:
                        ...
                        throw new Exception(
                            sprintf(
                                'JMTextRpcClient: Network %s unexpected problem: %s',
                                $rpcUri,
                                $execution_time,
                                $errInfo['message']
                            )
    ...
    public static function getSemFd()
    {
        ...
        {
            self::$semFd = sem_get(crc32(__FILE__));
        }
        return self::$semFd;
    }
    ...
    public static function getShmFd()
    {
        ...
        {
            self::$badAddressShmFd = shm_attach(crc32(__FILE__), 1024000);
        }
...
class ServiceClassWrapper{
    ...
    public function __call($method, $arguments)
    {
        $retry = $this->rpcClient->getConnectPoolCfgStatus() == true ? 2 : 1;
        for ($i = 0; $i < $retry; $i++) {
            try {
                // 如果第一次请求不成功，后续的请求会忽略连接池，进行直连.
                $conn = $this->rpcClient->openConnection($i == 0 ? true : false);
                ...
                if ($this->rpcClient->getRpcUri() == '') {
                    if ($this->rpcClient->getRpcService() == '') {
                        throw new \Exception('No service name provided');
                    }

                    // 在mcpd维护的文件中找特定服务的机器列表.
                    $this->rpcClient->setRpcUri($this->rpcClient->getHostByService($this->rpcClient->getRpcService()));
                    if ($this->rpcClient->getRpcUri() == '') {
                        throw new \Exception('Failed to get service list(' . $this->rpcClient->getRpcService() . ')');
                    }
                }
            }
        }
