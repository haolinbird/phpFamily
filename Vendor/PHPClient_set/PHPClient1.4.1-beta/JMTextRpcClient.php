...
class JMTextRpcClient
{
    ...
    public function init(array $config)
    {
        ...
        // 是否使用连接池.
        if (defined('MPC_SERVER') && MPC_SERVER != '' && $config['use_connect_pool'] == true) {
    ...
    public function openConnection()
    {
        ...
        if ($this->isUseConnectPool == true) {
            $rpcServers[] = MPC_SERVER;
        }
        $rpcServers[] = $this->rpcUri;

        while ($rpcUri = array_shift($rpcServers)) {
            $conn = @stream_socket_client($rpcUri, $errno, $errstr, static::CONN_TRY_TIMEOUT);
            ...
            if(!$conn && $errno == 110)
            {
                ...
                if($timeout > 0)
                {
                    $conn = stream_socket_client($rpcUri, $errno, $errstr, $timeout);
                }
            }
            if (!$conn) {
                $this->handleBadConnection();
                if (! $rpcServers) {
                    throw new Exception(sprintf('JMTextRpcClient: %s, %s (%.3fs)', $rpcUri, $errstr, $this->executionTime()));
                }
    ...
    */
    protected function sendData(array $data, $conn)
    {
        ...
        if ($this->isUseConnectPool == true || $this->rpcService) {
            $data['service'] = $this->rpcService;
        }
