...
class JMTextRpcClient
{
    ...
    {
        $this->connection = @stream_socket_client($this->rpcUri, $errno, $errstr, 4);
        if (!$this->connection) {
        ...
        $config = self::config();
        $recv_time_out = !empty($config['recv_time_out']) && (int)$config['recv_time_out'] > 0 ? (int)$config['recv_time_out'] : 23;
        @stream_set_timeout($this->connection, $recv_time_out);
    }
