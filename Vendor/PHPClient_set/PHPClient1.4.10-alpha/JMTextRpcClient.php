...
class JMTextRpcClient {
    ...
    protected $currentConnectionAddr = "";
    /**
     * 设置或读取配置信息.
     *
	...
	$configName = is_array($config)? '' : $config;
        if (empty($instances[$key]) || PHP_SAPI == 'cli') {
            ...
            if (strpos($className, 'RpcClient_') === 0) {
                ...
                $instances[$key] = new $className($configName);
            }

            $instances[$key]->rpcClass = $className;
        }
        
	$instances[$key]->configName = $configName;
    ...
    public function init(array $config) {
        ...
        if (!$this->useLark && !isset($this->config['service'])){
	    $this->config['service'] = $this->configName;
        }
    ...
    public function openLarkConnection() {
        ...
        if($envLarkAddr = getenv('JM_LARK_ADDRESS')){
            $proxyAddress = $envLarkAddr;
        }
        if (defined(('LARK_ADDRESS'))) {
            $proxyAddress = LARK_ADDRESS;
        }

        $this->currentConnectionAddr = $proxyAddress;
    ...
    */
    protected function dealCtx($ctx) {
        ...
        if (is_array($ctx) && isset($ctx['exception']) && is_array($ctx['exception'])) {
            $exStr = var_export($ctx['exception'], true)."\n"."rpc context: ".json_encode($ctx);
    ...
    */
    ...
    protected function getConnErr($conn) {
        ...
        if(feof($conn)) {
           return "链接断开: ".$this->currentConnectionAddr;
        }
        if ($meta['timed_out']) {
            return sprintf('读取服务端(%s)数据超时(%d秒)', $this->currentConnectionAddr, $this->recvTimeout);
        }
        ...
        try {
            ...
            try {
                ...
                try{
                    ...
                    throw new \Exception("尝试使用MCPD失败: ".$mcpEx->getMessage(), 0, $e);
                }
            }
    ...
    public function openMcpdConnection() {
        ...
        $this->currentConnectionAddr = $mcpdAddress;
        if (!$conn) {
    ...
    public function openServerConnection() {
        ...
        $this->currentConnectionAddr = $address;
