...
class JMTextRpcClient
{
    ...
    *@var bool
    */
    protected $_isAsyn = false;

    /**
    *异步调用方向,发送或者接收
    *@var string (send|recv)
    */
    protected $direction = 'SEND';
    ...
    */
    ...
    protected function openConnection()
    {
        ...
        $first_timeout = 1;
        $this->connection = @stream_socket_client($this->rpcUri, $errno, $errstr, $first_timeout);

        // 如果连接超时，尝试重连一次
        if(!$this->connection && $errno == 110)
        {
            $timeout = (int)(self::$connectionTimeOut - $first_timeout);
            if($timeout > 0)
            {
                $this->connection = @stream_socket_client($this->rpcUri, $errno, $errstr, $timeout);
            }
        }
        if (!$this->connection) {
    ...
    {
        //异步请求
        if ($this->_isAsyn && $this->direction == 'SEND') {
            $this->rpcMethod = $method;
            $this->mustSend($this->initRpcData($arguments));
            $this->direction = 'RECV';
            return $this;
        }
        ...
        //同步发送接收
        $this->rpcMethod = $method;
        $packet = $this->initRpcData($arguments);
        $this->mustSend($packet);
        return $this->recv();
    }
    ...
    *异步接收数据
    */
    public function asynRecv()
    {
        if ($this->_isAsyn && $this->direction == 'RECV') {
            return $this->recv();
        } else {
            throw new \Exception("The async send method: '{$this->rpcMethod}' has not been callded before.");
        }
    }
    ...
    */
    protected function mustSend($packet)
    {
        try {
            // uri 参数其实不需要，这里方便调用超时打印出调用栈，看到uri参数
            $this->sendData($packet, substr($this->rpcUri, 6));
        } catch (\Exception $e) {
            $trace_logger = null;
            $this->initTraceLogger($trace_logger);
    ...
    *@return array   待发送数据
    */
    protected function initRpcData($arguments)
    {
        $packet = array(
            'data' => json_encode(
                array(
        ...
        $trace_logger = null;
        $config = $this->initTraceLogger($trace_logger);

        $trace_logger && $trace_logger->RPC_CS(substr($this->rpcUri, 6), $this->rpcClass, $this->rpcMethod, $arguments);
    ...
    */
    protected function dealCtx($ctx)
    {
        ...
        if (is_array($ctx) && isset($ctx['exception']) && is_array($ctx['exception'])) {
            ...
            throw new Exception('RPC Exception: '.$this->rpcClass.'::'.$this->rpcMethod.' URI：' . $this->rpcUri . ' return ' . var_export($ctx['exception'], true));
        }
    ...
    *初始化日志对象
    *
    *@param objec $trace_logger 引用类型,日志对象
    *
    *@return array  $config
    */
    protected function initTraceLogger(&$trace_logger = null)
    {
        $config = self::config();
    ...
    */
    protected function sendData(array $data)
    {
        ...
        $len = @fwrite($this->connection, $buffer);
