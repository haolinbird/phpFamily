...
/**
 * Create at 2019年12月26日 by qiangd <qiangd@jumei.com>.
 */
class BaoFu extends \Utils\Singleton
{

    /**
     * Get instance of the derived class.
     *
     * @return \Utils\VerifyBankCard\BaoFu
     */
    ...
    public function auth(array $params)
    {
        try {
            ...
            $sceneCode = isset($params['scene_code']) ? $params['scene_code'] : '99'; // 详见 : https://docs.BaoFu.com/docs/bankcard_auth/fb?token=vVL8n8ZAGs4N
            $mobile = $params['mobile'];
            $customerName = isset($params['customer_name']) ? $params['customer_name'] : '';
            $request = array(
                'trans_id' => $transId,
                'member_id' => \Config\BaoFu::MEMBER_ID,
                'terminal_id' => \Config\BaoFu::TERMINAL_ID,
                ...
                'industry_type' => \Config\BaoFu::INDUSTRY_TYPE
            );
            if (!empty($cardType)) {
                $request['card_type'] = $cardType;
            }
            $post = $this->getPostData($request);
            $response = $this->request($post, \Config\BaoFu::API_HOST . '/bankcard/v3/auth');
    ...
    public function authsms(array $params)
    {
        try {
            ...
            $post = $this->getPostData($request);
            $response = $this->request($post, \Config\BaoFu::API_HOST . '/bankcard/v1/authsms');
            $res = json_decode($response, true);
            \Utils\Log\Logger::instance()->log($res);
            return $res;
        } catch (\Exception $e) {
    ...
    public function authconfirm(array $params)
    {
        try {
            ...
            $post = $this->getPostData($request);
            $response = $this->request($post, \Config\BaoFu::API_HOST . '/bankcard/v1/authconfirm');
            $res = json_decode($response, true);
            \Utils\Log\Logger::instance()->log($res);
            return $res;
        } catch (\Exception $e) {
    ...
    public function getPostData($request)
    {
        ...
        $pfxPath = realpath(\Config\BaoFu::RSA_PATH) . DIRECTORY_SEPARATOR . \Config\BaoFu::PFX_FILE;
        $cerPath = realpath(\Config\BaoFu::RSA_PATH) . DIRECTORY_SEPARATOR . \Config\BaoFu::CER_FILE;
        $pfxPwd = \Config\BaoFu::PASSWORD;
    ...
    public function idCardAuth(array $params)
    {
        try {
            ...
            $idCard = $params['id_card'];
            $idHolder = $params['id_holder'];
            $isPhoto = isset($params['is_photo']) ? $params['is_photo'] : 'noPhoto';
            $request = array(
                'trans_id' => $transId,
                'member_id' => \Config\BaoFu::MEMBER_ID,
                'terminal_id' => \Config\BaoFu::TERMINAL_ID,
            ...
            $post = $this->getPostData($request);
            $response = $this->request($post, \Config\BaoFu::API_HOST . '/idcard/v2/auth');
            $res = json_decode($response, true);
            \Utils\Log\Logger::instance()->log($res);
            return $res;
        } catch (\Exception $e) {
