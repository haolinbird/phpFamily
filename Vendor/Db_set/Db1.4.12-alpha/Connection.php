...
class Connection
{
    ...
    const WRITE = 'write';
    /**
     * @var string 连接的服务器是epg还是mysql.
     */
    protected $nodeTypeString = '"mysql"';
    /**
    ...
    protected function __construct($config = null)
    {
        ...
        if(!static::$configs['disableConnectionPoolProxy']){
            $this->nodeTypeString = "连接池代理epg";
        }
        if (!is_null($config)) {
            $this->connect($config);
        }
    ...
    public function read($name = 'default') {
        ...
        try {
            ...
            throw new \Exception("尝试连接所有的{$this->nodeTypeString} " . $this->type .'节点失败， 错误如下：' . PHP_EOL . $e->getMessage(), $e->getCode());
        }
    ...
    protected function getConnection($name) {
        ...
        for ($i = 0; $i < $nodesCount; $i++) {
            ...
            try {
                ...
                $expMsgs .= "连接{$this->nodeTypeString}节点[{$cfg['dsn']}]错误: " . $e->getMessage() . PHP_EOL;

                // 2002:connection refused 或者 connection timeout
                // 非2002是其它问题（鉴权错误等）,这类错误就不用重试，也不用加黑名单，直接报错
                // 可能是PHPServer抛出的超时异常，所以需要重新封装异常，否则无法知道报错的节点信息
                if ($e->getCode() != 2002) {
