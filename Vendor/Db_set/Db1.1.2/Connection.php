...
class Connection {
        ...
        if ($this->connection) {
            ...
            try {
                if ($this->usePool) {
                    ...
                    {// 当新获取的连接带有事务时,有可能是上次使用事务的进程被强行退出.
    ...
    public function beginTransaction($global = false) {
        ...
        if (!$this->usePool && !$this->connection->inTransaction()) {// Re-connect before begin a transaction. If inTransaction then skip this step to avoid breaking nested transactions.
            $this->reConnect();
        }
        $this->connection->beginTransaction();
        $this::$inTrans = true; //在事务里面持有其他连接，这个连接也不释放（即:一次release这个进程持有的所有连接都释放了）
    }
    ...
    public function commit($global = false) {
        if ($this->inGlobalTransaction && !$global) {// Prevent committing a global transaction unexpectedly in a normal transaction.
            return true;
        } else {// Ready to commit the global transaction, or there are no global transactions.
