...
class ShardingConnection extends Connection
{
    ...
    protected static $lastConn;    // 上一次使用的连接

    protected static $shareConn = false;    // 是否共用一个连接

    protected static $currentDatabase = ''; // 当前操作的DB
...
//          $db = $rule->getDbName($db);
     ...
     * @return boolean
     */
    public function isShareConn()
    {
        return self::$shareConn;
    }

    /**
     * @param boolean $shareConn
     */
    public function setShareConn($shareConn)
    {
        self::$shareConn = (bool)$shareConn;
    }
    ...
    public function read($rule = null)
    {
        ...
        if (isset(self::$configs['read'][$name])) {
            ...
            $conn = null;
            //使用共享连接
            if (self::$shareConn === true && self::$lastConn !== null){
                $conn = self::$lastConn;
            } else {
                //使用缓存的连接或新建
                if (empty(self::$readConnections[$cfg['alias']]) && ! $this->addReadConnection($rule)) {
            ...
            //check the database is same?
            if (self::$currentDatabase !== $cfg['db']){
                $re = $conn->exec("USE {$cfg['db']}", false);
                if ($re !== true){
                    throw new Exception('use database "' . $cfg['db'] . '" failed.', 42004);
                }
                self::$currentDatabase = $cfg['db'];
            }
            return $conn;
        } else {
    ...
    public function write($rule = null)
    {
        ...
        if (isset(self::$configs['write'][$name])) {
            ...
            if (self::$shareConn === true && self::$lastConn !== null){
                ...
                //使用缓存的连接或新建
                if (empty(self::$writeConnections[$cfg['alias']]) && !$this->addWriteConnection($rule)) {
    ...
    public function addReadConnection( $rule = null)
    {
        ...
        if (isset(self::$configs['read'][$name])) {
            ...
            self::$lastConn = $connection;
            self::$currentDatabase = $cfg['db'];
            self::$readConnections[$cfg['alias']] = $connection;
            return self::$readConnections[$cfg['alias']];
        } else {
            throw new Exception('Read configuration of "' . $name . '" is not found.', 42003);
        }
    }
    ...
    public function addWriteConnection( $rule = null)
    {
        ...
        if (isset(self::$configs['write'][$name])) {
            ...
            return self::$writeConnections[$cfg['alias']];
        } else {
            throw new Exception('Write configuration of "' . $name . '" is not found.', 42003);
        }
    }
