...
class Connection
{
    ...
    public function addReadConnection($name = 'default') {
        if (isset(self::$configs['read'][$name])) {
            ...
            //客户端负载模式, 选取访问节点
            $cfg = $this->getConfig(self::$configs['read'][$name], 'read');
    ...
    public function addWriteConnection($name = 'default') {
        if (isset(self::$configs['write'][$name])) {
            //客户端负载模式, 选取访问节点
            $cfg = $this->getConfig(self::$configs['write'][$name], 'write');
            $connection = new self($cfg ['dsn'], $cfg ['user'], $cfg ['password'], $cfg ['options']);
            $connection->connectionCfg = $cfg;
     ...
     * @return array|mixed
     */
    public function getConfig($config, $type ='read'){
        $cfg = array_shift($config);
        $cfg['dc'] = 'default';
        $dcCfg = (array) self::$dataCenter->getDataCenterCfg($cfg['db'], 0, $type);
        if (!empty($dcCfg)) {
            $dcCfg = $this->getOneNode($dcCfg);
            //overwrite the dsn, for the dsn is the $globalDSN
            $dcCfg['dsn'] = "mysql:host={$dcCfg['host']};port={$dcCfg['port']};dbname={$cfg['db']}";
        }
    ...
    /**
    * 说明：按权重随机數組中的一個
    * @param array ( array(..., weight => 123),...)
    * @return array $arr 
    */
    public function getOneNode(array $arr){
