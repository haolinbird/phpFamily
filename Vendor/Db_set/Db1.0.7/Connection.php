...
class Connection
{
    ...
    public function isQueryError($errorCode)
    {
        ...
        return !isset($this::$noQueryErrorCodeHeaders[$codeHeader]);
    }
    ...
    public static function getExternalCaller()
    {
        ...
        {
            if(isset($line['class']) && in_array($line['class'], array(__CLASS__, 'CCacheManager_Memcached')))
            {
                continue;
            }
    ...
    /**
     * 记录Exception至监控日志。需在\Config\MNLogger中配置exception一项。
     *
     * @param \Exception $ex
     *
