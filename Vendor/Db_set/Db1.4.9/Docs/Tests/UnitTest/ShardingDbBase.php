...
class DbBase extends Base{
    ...
    public function __get($name)
    {
        switch ($name)
        {
            default:
                ...
                throw new \Db\Exception('Try get undefined property "'.$name.'" of class '.get_called_class().'. Forgot to call fillFields ?');
                continue;
        }
    }
    ...
    public function fillFields(array $values)
    {
        ...
        {
            throw new \Db\Exception('You cannot call this method, $fields is not an array or is empty in class '.get_class($this).'!');
        }
        $this->content = array();
        foreach ($values as $k => $v)
        {
            ...
            {// 兼容0.1.0d的判断
                throw new \Db\Exception('Try to fill a field "'.$k.'" that is not defined in property "fields" of Model '.get_class($this).'. Is it a typo ?');
            }
            else
            {
                $this->content[$k] = $v;
    ...
    public function db($name=null, $type='read')
    {
        ...
        {
            throw new \Db\Exception('Db type "'.$type.'" is not valid!');
        }
    ...
    public function save(array $fieldValues = array())
    {
        ...
        {
            throw new \Db\Exception('Empty fields to save !');
        }
