...
class Connection {
    ...
    protected function __construct($dsn = null, $username = null, $passwd = null, $options = array()) {
        ...
        $this->connectionCfg = array('dsn' => $dsn, 'username' => $username, 'passwd' => $passwd, 'options' => $options);
        if (!self::$configs) {
    ...
    * @return bool
    */
    public static function isUsePool() {
        $use = false;
        if(defined('JM_PHP_MYSQL_POOL_ON')) {
            //if begin with "#", that's the template parse error with not found key.
            if (is_string(JM_PHP_MYSQL_POOL_ON) && JM_PHP_MYSQL_POOL_ON[0] == '#' ) {
                $use = false;
            } else {
                $use = (bool) JM_PHP_MYSQL_POOL_ON;
            }
        }
     ...
     * 原始格式如："mysql:dbname=tuanmei_operation;host=192.168.20.71:9001:1,192.168.20.72:9001:1"
     * @param $cfg 数据库的原始配置.
     * @return array 一个或多个包含标准dsn配置 如: 
     * array(
     *   0 => array('dsn'=>'dbname=testdb;host=127.0.0.1;port=3306', 'weigth'=>1), 
     *   1 => array('dsn'=>'dbname=testdb;host=127.0.0.2;port=3306', 'weigth'=>2)
     * );
     * 除以上信息外还会保留原来的额外配置.
     * @throws \Db\Exception
     */
    public static function parseCfg($cfg) {
        if (!isset($cfg['dsn'])) {// 没有配dsn，可能已经手动拆分好了各节点.             
            return $cfg;
        }
        $dsn = $cfg['dsn'];
        unset($cfg['dsn']);
        ...
        foreach ($hosts as $host) {
            ...
            $parsed[] = array_merge($cfg, array('dsn' => $parsedDsn, 'weight' => $weight, 'host' => $hostPortions[0], 'port' => $port));
        }
        return $parsed;
    }
            ...
            if (!isset(self::$fmtConfigs['read'][$name])){
                ...
                if (!isset( $_cfg['dsn'])){
                    if (isset($_cfg['host'])){
                        ...
                        self::$fmtConfigs['read'][$name] = $_cfg; 
                    }
                }
            }
        ...
        if ($this->connection) {
            ...
            try {
                if ($this->usePool) {
                    ...
                    $options[\PDO::MYSQL_ATTR_USE_BUFFERED_QUERY] = true;//set use buffered query
    ...
    public function release($force = false) {
        if ($this->usePool && $this->connection && ($this::$inTrans==0 || $force)) {
