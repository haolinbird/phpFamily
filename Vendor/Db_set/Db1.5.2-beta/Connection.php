...
class Connection {
	...
	const SHADOW_DB_PREFIX = 'shadow_';
	...
	*/
	 ...
	 * @var String
	 */
	protected $configBeForePartition;
	...
	const EPG_RECONNECT_STATUS = 7001;
	const EGP_BAD_CONNECTION_NODE_TTL = 20;

	/**
	 * <pre> 设置分区(标识)参数，准备根据UID来获取不同分区的链接.
	 * 一般情况下,同一个用户(UID)的所有相关数据,都位于同一个分区之下.
	 * 调用partitionByUID($uid)后, 调用read()或write()时，返回UID对应分区的连接，同时重置分区标识.
	 * （当UID等于0时，即使用默认分区)。
	 * 用法: \Db\Connection::instance()->partitionByUID(211)->write('user_info')->query(...)
	 * \Db\Connection::instance()->write('user_info')->query(...)
	 * 或 $db = \Db\ShardingConnection::instance()-->partitionByUID(211)->write(new \UserShardingRule())
	 * </pre>
	 * @param int $uid
	 * @return $this
	 */
	public function partitionByUID($uid) {
		// 判断uid参数是否为非负整数
		$uid = (string) $uid;
		if (!ctype_digit($uid) || $uid < 0) {
			throw new \Exception('illegal partition uid:' . $uid);
		}
		...
		// 计算uid对应的分区标识
		foreach ($partitionDsnConfig['partition']['dsn'] as $partitionIndex => $config) {
			// 如果已经选出分区标识则退出检测
			if ($this->partitionIndex !== '') {
				break;
			}

			// 如果缺少配置抛出异常
			if (!isset($config['uid_range'])) {
				throw new \Exception('miss parttiondsn uid_range config');
			}

			// 判断uid是否属于该分区
			foreach ($config['uid_range'] as $uidZone) {
				if (!is_array($uidZone) || count($uidZone) != 2) {
					throw new \Exception('illegal parttiondsn uid_range config');
				}

				list($min, $max) = $uidZone;
				if ($uid >= $min && $uid <= $max) {
					// 命中uid区间配置,返回对应分区标识
		...
		// 如果uid没有命中任何分区, 说明目前没有创建存储该uid数据的分区, 抛出异常
		if ($this->partitionIndex === '') {
			throw new \Exception('no partition support this uid:.' . $uid);
		}
		...
		// 使用分区dsn覆盖原有config配置
		static::$configs = \Db\ConfigSchema::replaceGlobalDsnCfg(static::$configs, $this->partitionIndex);
	...
	/**
	 * 获取分区及对应的UID区间。<br />
	 * @return array 例如：$partitionInfo = array( 0 => [[0, 10000], [3000,5000]], 1 => [2000,29999])
	 */
	public function partitionInfo() {
		$info = array();
		if (!isset(static::$configs['partitionDsn']['partition']['dsn'])) {
			return $info;
		}

		foreach (static::$configs['partitionDsn']['partition']['dsn'] as $n => $p) {
			$info[$n] = isset($p['uid_range']) ? $p['uid_range'] : array();
		}
	...
	/**
	 * <pre>
	 * 如果使用了分区dns配置,就恢复原有配置
	 * </pre>
	 *
	 * @return void
	 */
	public function toRestoreDsnConfig() {
		// 重置static::$configs配置
		if ($this->configBeForePartition) {
			static::$configs = $this->configBeForePartition;
			$this->configBeForePartition = null;
		}
	 ...
	 * @todo connection name select
	 */
	public function readAll($name = 'default') {
		return $this->readWriteAll($name, 'read');
	}
	 ...
	 * @return \Db\Connection
	 */
	public function writeAll($name = 'default') {
		return $this->readWriteAll($name, 'write');
	}
	 ...
	 * @return \Db\Connection
	 */
	private function readWriteAll($name, $type) {
		if ($name == 'default' && func_num_args() == 0 && $this->defaultConfigName) {
			$name = $this->defaultConfigName;
		}
		...
		if (isset(static::$configs['partitionDsn'])) {
			...
			foreach ($partitionDsnConfig['partition']['dsn'] as $partitionIndex => $partitionDsnConfig) {
				...
				try {
					$allConnections[] = $this->getConnection($name);
				} catch (\Exception $e) {
					throw new \Exception('尝试连接所有的epg ' . $this->type . '节点失败， 错误如下：' . PHP_EOL . $e->getMessage(), $e->getCode());
				} finally {
					// 重置partition dsn配置,避免下个请求拿到错误的dsn配置
	...
	protected function getConfig($configs, $dcConfigs, $rule = null) {
		...
		// 再考虑一下，单例模式下是否存在污染生产实例的风险。链接对象的key已经包含了db，而db是不同环境不相同，因此不存在问题.
		$config['db'] = $this->tryShadowDb($config['db']);
		...
		$config['alias'] = "{$this->partitionIndex}:{$config['dc']}:{$config['db']}:$name:" . $this->type;
	 ...
	 * 如果是压测环境，需要将db设置为影子库
	 */
	protected function tryShadowDb($db) {
		global $context;
		// 压测环境改为影子库
		if (@$context['X-Jumei-Loadbench'] == 'bench') {
			return self::SHADOW_DB_PREFIX . $db;
		}
		return $db;
	}
	...
	public function connect($config) {
		...
		if (isset(static::$configs['DEBUG']) && static::$configs['DEBUG'] && isset(static::$configs['DEBUG_LEVEL']) && static::$configs['DEBUG_LEVEL'] == 2) {
			$logString = 'connecting with dsn:' . $config['dsn'] . "\n";
			$tempE = new \Exception();
	 ...
	 * @return \Db\Connection
	 **/
	protected function reConnectEpg() {
		$this->close();
		$rule = (isset($this->currentRule) && !empty($this->currentRule)) ? $this->currentRule : null;
		$key = $this->connectionCfg['host'] . ':' . $this->connectionCfg['port'];
		self::$badConnectionAlias[$key] = time() . ':' . self::EGP_BAD_CONNECTION_NODE_TTL;
		$temp = explode(':', $this->connectionCfg['alias']);
		$type = array_pop($temp);
		if ($type) {
			$this->type = $type;
		}
		$name = array_pop($temp);
		if (!$rule) {
			$configs = self::$configs[$type][$name];
			$configs['name'] = $name;
			$db = $this->getFieldFromConfigs($configs, 'db');
			$dcConfig = (array) self::$dataCenter->getDataCenterCfg($db, 0, $type);
			$cfg = $this->getConfig($configs, $dcConfig, $rule);
			$this->connect($cfg);
			$this->connectionCfg = $cfg;
		} else {
			$configs = $this->getShardingConfig($rule, $type, $name);
			$this->connect($configs);
			$this->connectionCfg = $configs;
		}
	...
	public function insert($table, $params, $onDup = null) {
		...
		foreach ($params as $column => $value) {
			list($column, $isExpress) = Util::fieldValueIsMysqlExpress($column);
			if ($isExpress) {
		...
		if (empty($onDup)) {
			...
			foreach ($update_params as $column => $value) {
				if (is_int($column)) {
					...
					list($column, $isExpress) = Util::fieldValueIsMysqlExpress($column);
					if ($isExpress) {
						$updates[] = $this->quoteObj($column) . "=" . (is_null($value) ? "null" : $value);
						continue;
					}
			...
			foreach ($params as $column => $value) {
				...
				if (!in_array($column, $noUpdateColumnNames)) {
					...
					if ($isExpress) {
						$updates[] = "$column=" . (is_null($value) ? "null" : $value);
						continue;
					}
	...
	public function query($sql = null) {
		...
		if ($this->allowRealExec || $sqlCmd == 'SELECT') {
			...
			try {
// 连接池在查询失败时也会抛异常,而pdo在默认情况下会返回false.
		...
		if (false === $this->lastStmt) {
			...
			if ($retryCount < 3 && $this->needConfirmConnection()) {
				$connectionLost = 2006 == $errorInfo[1] || self::EPG_RECONNECT_STATUS == $errorInfo[1];
				if ($connectionLost) {
					$retryCount += 1;
					$this->reConnectEpg();
...
// Prevent committing a global transaction unexpectedly in a normal transaction.
			return true;
		} else {
...
// Ready to rollback the global transaction.
	...
	public function buildCondition($condition = array(), $logic = 'AND') {
		if (!is_array($condition)) {
			if (is_string($condition)) {
				if (Util::isMysqlFuncCall($condition)) {
					return $condition;
				}
		...
		foreach ($condition as $k => $v) {
			...
			// 判断当前字段的值是否是mysql表达式
			list($k, $isExpress) = Util::fieldValueIsMysqlExpress($k);
			$k_upper = strtoupper($k);
	...
	public function logQuery($sql) {
		if (isset(static::$configs['DEBUG']) && static::$configs['DEBUG'] && isset(static::$configs['DEBUG_LEVEL'])) {
			$logString = 'Dsn:' . $this->connectionCfg['dsn'] . "\n";
...
// 可能在eval或者call_user_func里调用的。
	...
	public function getPageResultByIndex($pageIndex, $rowsPerPage, $countColumnsOrSqlForCount = '*', $sqlForQueryWithoutLimit = null, $primaryKey = '', $sort = 'ASC') {
		...
		if ($sqlForQueryWithoutLimit) {
			...
			// no $sqlForCount, use the chain sql mode
	...
	public function getOneNode(array &$arr, $unsetChoosed = false) {
		...
		foreach ($weight as $k => $v) {
			if ($one <= $v) {
				...
				$key = $ret['host'] . ':' . $ret['port'];
				if (isset(self::$badConnectionAlias[$key])) {
					list($time, $ttl) = explode(':', self::$badConnectionAlias[$key]);
					if ((time() - $time) < $ttl) {
						$one = mt_rand($v, $count);
						continue;
					}
