...
class EXLogger extends Base {
    ...
    {
        $config = static::getRecommendConfig('exception2');
    ...
    // 添加全链路日志.
    public function trace2Log($type, $exception)
    {
        if ($this->_on === self::OFF) {
            return;
        }
        ...
        if ($exception instanceof \Exception) {
            ...
            $error_msg = $this->serializeData($exception);
        } else {
            $error_msg = $exception;
        }
        ...
        // debug 错误内容为空的问题.
        if (empty($error_msg)) {
            $ex = new \Exception('errmsg is null');
            $msg = $ex->getMessage();
            $file = $ex->getFile();
            $line = $ex->getLine();
            $error_msg = "{$msg} in {$file} on line {$line}:\n". $ex->getTraceAsString(). "\n";
            // $error_msg = "RAW INFO:$exception;TRACE:" . var_export(debug_backtrace(DEBUG_BACKTRACE_PROVIDE_OBJECT, 10), true);
        }

        $time = date('Y-m-d H:i:s');
        $line = "OWL\001DATA\0010002\001{$this->_app}\001{$time}.000\001{$this->_ip}\001Exception\001{$traceId}\001{$spanId}\001{$type}\001{$error_msg}\004\n";
